  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="drawer" role="dialog" aria-modal="true" aria-label="Item editor">
      ...
      <div className="editor-scroll">
        ...
        <details open={item.state !== 'open'}><summary>Status</summary><div className="details-body"><label>Item status<select value={item.state} onChange={(event) => patchItem({ state: event.target.value as UniversalItem['state'] })}>{['open', 'done', 'cancelled', 'auto_closed', 'archived'].map((state) => <option key={state}>{state}</option>)}</select><small>Status normally changes through completion, cancellation, auto-renew or archiving.</small></label></div></details>
...
function SavedViewSection({ view, workspace, onEditView, onEditItem, onState, onExport }: {
  ...
  return <details className="view-section" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}>
    ...
    <div className="view-section-body"><details className="view-query-details"><summary>View details</summary><div className="view-query-details-body"><code>{view.query.source.trim() || 'All items'}</code>{(view.sortSource || view.sort?.length) && <code className="sort-preview">Sort: {view.sortSource ?? view.sort.map((sort) => `${sort.field} ${sort.direction}`).join(' · ')}</code>}<p>{matchingItems} matching items</p></div></details><div className="view-export-actions"><span>Export</span><button onClick={() => onExport('definition')}>Definition</button><button onClick={() => onExport('results')}>Results</button><button onClick={() => onExport('bundle')}>Definition + results</button></div><ViewResults view={view} workspace={workspace} onEdit={onEditItem} onState={onState} /></div>
  </details>;
}
