...
function ViewsPage({ workspace, commit, onEditItem, onState, onOpenCalendar, onAddItem, celebratingIds }: {
  ...
  onOpenCalendar?: (viewId: string) => void; onAddItem: (view: SavedView) => void; celebratingIds?: ReadonlySet<string> | undefined;
}) {
  ...
  return <section className="page-section views-page">
    ...
    {editing && <div className="modal-backdrop"><section className="dialog view-editor">
      ...
      <fieldset className="view-accent-picker"><legend>View color</legend><p className="builder-status">This color identifies the view and completed ticks. Each option stays readable in light and dark themes.</p><div className="view-accent-options"><button type="button" className={!editing.accent ? 'selected' : ''} onClick={() => { const { accent: _accent, ...withoutAccent } = editing; setEditing(withoutAccent); }}>Default</button>{viewAccentOptions.map((option) => <button type="button" key={option.value} className={editing.accent === option.value ? 'selected' : ''} aria-label={`${option.label} view color`} aria-pressed={editing.accent === option.value} onClick={() => setEditing({ ...editing, accent: option.value })}><span style={{ backgroundColor: option.value }} />{option.label}</button>)}</div></fieldset>
      ...
      <fieldset className="query-builder visual-query-builder"><legend>Visual setup</legend>
      ...
      <label className="dsl-field">Advanced filter code <span className="hint">Optional text form of the visual rows. SQL preview: {toSqlExpression(editing.query.source)}</span><CodeEditor language="dsl" ariaLabel="Advanced filter code" rows={5} value={editing.query.source} onChange={(value) => { const rows = parseVisualRows(value); setEditing({ ...editing, query: { source: value } }); if (rows !== null) setVisualRows(rows); setVisualDirty(rows === null); }} /></label>
      <fieldset className="query-builder creation-defaults"><legend>Defaults for new items</legend>
...
export default function App() {
  ...
  const activeDateLabel = `${wallClock.toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric' })} · ${wallClock.toLocaleTimeString(undefined, { hour: '2-digit', minute: '2-digit' })}`;
  const activeCountLabel = openItems ? `${openItems} active ${openItems === 1 ? 'item' : 'items'}` : 'Everything is clear';
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {page === 'home' && <><ViewsPage workspace={workspace} commit={commit} onEditItem={(item) => setEditor(itemEditorSource(workspace, item))} onState={changeItemState} celebratingIds={celebratingIds} onAddItem={(view) => { setEditorIsNew(true); setEditor(applyViewCreationDefaults(createUiItem('', 'task'), view)); }} /></>}
