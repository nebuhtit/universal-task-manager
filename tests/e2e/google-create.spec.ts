...
const expand = async (summary: Locator) => {
  if (!await summary.evaluate(element => element.parentElement?.hasAttribute('open'))) await summary.press('Space');
...
test('edits one Google occurrence with conflict recovery and keeps local journals', async ({ page }) => {
  ...
  await page.locator('article').getByText('Changed in UTM', { exact: true }).first().click();
   await expect(properties.getByText('Measured preparation', { exact: true })).toBeVisible();
  ...
  await expand(itemEditor.locator('[data-editor-section="dates"] > summary'));
  await expand(itemEditor.locator('summary').filter({ hasText: /^Progress & completions/ }));
...
test('saves one linked event directly and recovers a lost response', async ({ page }) => {
  test.setTimeout(120_000);
  let inserts = 0; let patches = 0; let moves = 0; let deletes = 0;
  ...
  await page.route('https://www.googleapis.com/calendar/v3/**', async (route) => {
    ...
    if (request.method() === 'DELETE') {
      expect(url).toContain(`/events/${created!.id}`);
      // Reload may interrupt local acknowledgement: retrying the same remote
      // identity must be harmless, not counted as another deletion effect.
      if (deletes) return route.fulfill({ status: 410, json: {} });
      deletes++; return route.fulfill({ status: 204 });
    }
  ...
  await expand(editor.locator('[data-editor-section="dates"] > summary'));
  ...
  await expand(editor.locator('[data-editor-section="dates"] > summary'));
  const duration = await editor.getByLabel('Calendar duration amount', { exact: true }).inputValue();
  const durationUnit = await editor.getByLabel('Calendar duration unit', { exact: true }).inputValue();
  await editor.getByLabel('Due / Active range ends', { exact: true }).fill('2030-09-24T13:00');
  await editor.getByRole('button', { name: 'Clear Event ends', exact: true }).click();
  await expect(editor.getByLabel('Event ends', { exact: true })).toHaveValue('');
  page.once('dialog', async dialog => { expect(dialog.message()).toContain('Delete the linked event'); await dialog.accept(); });
  await editor.getByRole('button', { name: 'Save item', exact: true }).click();
  await expect(editor).toBeHidden(); await expect.poll(() => deletes).toBe(1);
  await page.reload(); await page.getByLabel(/Безопасное открытие —/).uncheck();
  await page.getByLabel('Password', { exact: true }).fill('correct horse battery staple');
  ...
  await expand(editor.locator('[data-editor-section="dates"] > summary'));
  await expect(editor.getByLabel('Event ends', { exact: true })).toHaveValue('');
  await expect(editor.getByLabel('Calendar duration amount', { exact: true })).toHaveValue(duration);
  await expect(editor.getByLabel('Calendar duration unit', { exact: true })).toHaveValue(durationUnit);
  expect(inserts).toBe(2); expect(deletes).toBe(1);
  await page.keyboard.press('Escape'); await expect(editor).toBeHidden();
});
