...
test('shows the release version on registration, login and settings', async ({ page }) => {
  const releaseLabel = /^v2\.8\.11 · (?:local changes · )?commit [0-9a-f]{7}$/;
  ...
  await expect(page.locator('.settings-release-info')).toHaveText(/^Universal Task Manager · v2\.8\.11 · build [0-9a-f]{7}(?: · local changes)?(?: · .+)?$/);
