...
import {
  createId, evaluateFormulas, evaluateItemScripts, migrateItem, parsePortablePackage,
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="drawer" role="dialog" aria-modal="true" aria-label="Item editor" onKeyDown={(event) => {
      ...
      <div className="editor-scroll" ref={editorScrollRef}>
        ...
        <ItemSection sectionKey="template" title="Template" filledMark={sectionMark(isTemplate)}><Checkbox checked={isTemplate} onChange={(event) => setIsTemplate(event.target.checked)} label="Save this item as a template" /><p className="schedule-explainer">Templates are kept in the same workspace but do not appear in ordinary lists. They can be selected only while creating a new item.</p></ItemSection>
        ...
        <ItemSection sectionKey="priority" title="Priority" filledMark={sectionMark(Boolean(item.priority))} compact><Field label="Priority"><Select aria-label="Priority" value={item.priority ?? 0} onChange={(event) => patchItem({ priority: Number(event.target.value) as NonNullable<UniversalItem['priority']> })}>{([0, 1, 2, 3, 4] as NonNullable<UniversalItem['priority']>[]).map((priority) => <option key={priority} value={priority}>{priority ? `${priority} — ${priorityNames[priority]}` : 'None'}</option>)}</Select></Field></ItemSection>
        ...
        <ItemSection sectionKey="tags" title="Tags" filledMark={sectionMark(commaList(tags).length > 0)} compact><Field label="Tags"><Input aria-label="Tags" value={tags} onChange={(event) => setTags(event.target.value)} placeholder="Add tags separated by commas" /></Field>{collectedTags.length > 0 && <div className="tag-collection" aria-label="Available tags">{collectedTags.map((tag) => <Button size="compact" variant="ghost" className={commaList(tags).includes(tag) ? 'active' : ''} aria-pressed={commaList(tags).includes(tag)} key={tag} onClick={() => toggleTag(tag)}>#{tag}</Button>)}</div>}</ItemSection>
        <ItemSection sectionKey="task-link" title="Task link" filledMark={sectionMark(Boolean(item.list))} compact><Field label="Task list" optional><Input aria-label="Task list" value={item.list ?? ''} list="item-list-values" onChange={(event) => patchItem({ list: event.target.value.trim() || undefined })} placeholder="Optional list name" /></Field></ItemSection>
        ...
        <ItemSection sectionKey="status" title="Status" filledMark={sectionMark(item.state !== 'open')}><Field label="Item status" hint="Status normally changes through completion, cancellation, auto-renew or archiving."><Select aria-label="Item status" value={item.state} onChange={(event) => { const state = event.target.value as UniversalItem['state']; patchItem({ state, closure: state === 'open' ? undefined : { at: item.closure?.at ?? new Date().toISOString(), actor: item.closure?.actor ?? 'user', reason: state === 'cancelled' ? 'cancelled' : 'manual' } }); }}>{['open', 'done', 'cancelled', 'auto_closed', 'archived'].map((state) => <option key={state} value={state}>{stateNames[state as UniversalItem['state']]}</option>)}</Select></Field>{(item.state === 'done' || item.state === 'cancelled') && <label>Actually {item.state === 'done' ? 'completed' : 'cancelled'} at {dateField(`Actually ${item.state === 'done' ? 'completed' : 'cancelled'} at`, item.closure?.at, (value) => { if (value) patchItem({ closure: { at: value, actor: item.closure?.actor ?? 'user', reason: item.state === 'cancelled' ? 'cancelled' : 'manual' } }); else patchItem({ closure: undefined }); }, 'Defaults to now. Change this when you are recording the item after it happened. For a completion-anchored series, the next cycle uses this time when this cycle is first closed.')}</label>}</ItemSection>
        <ItemSection sectionKey="contexts" title="Contexts" filledMark={sectionMark(commaList(contexts).length > 0)}><Field label="Contexts"><Input aria-label="Contexts" value={contexts} onChange={(event) => setContexts(event.target.value)} placeholder="office, laptop" /></Field></ItemSection>
