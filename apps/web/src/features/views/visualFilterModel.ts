...
export const visualOptions: Record<string, string[]> = {
  ...
  isNote: ['true', 'false'], isHabit: ['true', 'false'], isTemplate: ['true', 'false'], isSavedTemplate: ['true', 'false'], isSubtask: ['true', 'false'], isParent: ['true', 'false'], activeRange: ['true', 'false'], activeRangeWhenSet: ['true', 'false'], activeRangeWhenSetOrOverdue: ['true', 'false'], activeDuration: ['true', 'false'], hasActiveReminders: ['true', 'false'], eventToday: ['true', 'false'], eventThisWeek: ['true', 'false'], dueTodayOrOverdue: ['true', 'false'], dueThisWeekOrOverdue: ['true', 'false'], isGoogleEvent: ['true', 'false'], googleCalendarAllDay: ['true', 'false'], 'schedule.allDay': ['true', 'false'], 'recurrence.autoRenew': ['true', 'false'], role: ['standalone', 'series_template', 'occurrence'], priority: ['0', '1', '2', '3', '4'], 'external.provider': ['google_calendar'], 'external.transparency': ['opaque', 'transparent'], 'external.readOnly': ['true', 'false'],
...
const organizationStorageField = (field: string) => field === 'area' ? 'areas' : field === 'project' ? 'projects' : field;
const organizationVisualField = (field: string) => field === 'areas' ? 'area' : field === 'projects' ? 'project' : field;

export const isOrganizationChoiceField = (field: string): boolean => organizationChoiceFields.has(field);
...
export const visualOperators = (field: string, customFields: Record<string, CustomFieldDefinition> = {}): string[] => {
  ...
  if (organizationChoiceFields.has(field)) return field === 'tags'
    ? ['is set', 'is not set', '==', '!=', 'has any', 'has all', 'has none']
    : ['is set', 'is not set', '==', '!='];
...
export const visualFilterValueLabel = (field: string, value: string, russian = false): string => {
  if (field === 'isGoogleEvent') return value === 'true' ? russian ? 'Да' : 'Yes' : russian ? 'Нет' : 'No';
...
const visualClause = (row: Pick<VisualConditionRow, 'field' | 'operator' | 'value'>, customFields: Record<string, CustomFieldDefinition> = {}) => {
  ...
  const filterField = organizationStorageField(row.field);
  const presenceExpression = fieldKind === 'multi' || fieldKind === 'text' ? `length(${filterField}) > 0` : `${filterField} != null`;
  ...
  if (organizationChoiceFields.has(row.field)) {
    ...
    if (values.length > 1) return `${all ? 'matchesAll' : positive ? 'matchesAny' : 'matchesNone'}(${filterField}, ${args})`;
    if (filterField === 'list') return `${filterField} ${positive || all ? '==' : '!='} ${args}`;
    return `${positive || all ? '' : '!'}includes(${filterField}, ${args})`;
  }
  if (fieldKind === 'multi') {
