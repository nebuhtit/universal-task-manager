...
export function ViewResults({ view, workspace, onEdit, onState, onReorder, celebrationColors = new Map<string, string>() }: {
  ...
  const changeState = (item: UniversalItem) => onState(item, nextState(item), view.accent ?? '#2f7d32');
  ...
  if (renderView.renderer === 'board') {
    ...
    return visibleColumns.length ? <div className="mini-board">{visibleColumns.map(({ key, label, items: columnItems }) => <section key={key}><h4>{label}</h4>{columnItems.map((item) => <article className={`board-item state-${item.state}${isCelebrating(item) ? ' is-celebrating' : ''}`} style={celebrationStyle(item)} key={item.id}><button className="state-toggle" aria-label={stateButtonLabel(item)} onClick={() => changeState(item)}>{isOpen(item) ? '' : '✓'}</button><button className="board-item-main" onClick={() => onEdit(item)}>{fieldContent(item, ['state'])}</button></article>)}</section>)}</div> : <p className="empty">No items match this board.</p>;
  }
  if (renderView.renderer === 'table') return <div className="table-wrap renderer-table-wrap"><table><thead><tr><th className="reorder-column"><span className="sr-only">Manual order</span></th><th className="state-column"><span className="sr-only">Complete</span></th>{visibleFields.map((field) => { const label = viewFieldLabel(renderWorkspace, field); return <th key={field} aria-label={label} title={label}><FieldIcon path={field} label={label} /><span className="sr-only">{label}</span></th>; })}</tr></thead><tbody>{items.map((item) => <tr data-view-item-id={item.id} className={`state-${item.state}${isCelebrating(item) ? ' is-celebrating' : ''}${draggingId === item.id ? ' is-dragging' : ''}${dropTargetId === item.id ? ' is-drop-target' : ''}`} style={celebrationStyle(item)} key={item.id} onClick={() => onEdit(item)}><td className="reorder-column">{dragHandle(item)}</td><td className="state-column"><button className="state-toggle" aria-label={stateButtonLabel(item)} onClick={(event) => { event.stopPropagation(); changeState(item); }}>{isOpen(item) ? '' : '✓'}</button></td>{visibleFields.map((field) => { const value = displayViewValue(readItemField(item, field, renderWorkspace, liveNow), field, renderWorkspace.calendarPreferences.language); return <td key={field} data-field={field} title={field === 'bodyMarkdown' ? value : undefined}>{value}</td>; })}</tr>)}</tbody></table></div>;
  return <div className="item-list reorderable-item-list">{items.map((item) => <div data-view-item-id={item.id} className={`reorderable-view-item${draggingId === item.id ? ' is-dragging' : ''}${dropTargetId === item.id ? ' is-drop-target' : ''}${isCelebrating(item) ? ' is-celebrating' : ''}`} style={celebrationStyle(item)} key={item.id}>{dragHandle(item)}<ItemCard item={item} celebrating={false} fields={visibleFields} workspace={renderWorkspace} now={liveNow} onEdit={() => onEdit(item)} onState={(state) => onState(item, state, view.accent ?? '#2f7d32')} /></div>)}</div>;
}
