...
export const BUILT_IN_VIEW_TEMPLATES: SavedView[] = [
  ...
  template('today-overdue', 'Today', `${active} && scheduleInPeriod("today", "event_open,event,active,due", true, 7, "", "") && activeRangeWhenSet == true`, '#c27a00'),
  template('tomorrow', 'Tomorrow', `${active} && scheduleInPeriod("tomorrow", "event_open,active,due", false, 7, "", "")`, '#6b7280'),
  template('week-overdue', 'This week', `${active} && scheduleInPeriod("this_week", "event_open,event,active,due", true, 7, "", "") && activeRangeWhenSet == true`, '#087f73'),
