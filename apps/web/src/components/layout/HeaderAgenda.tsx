...
export function HeaderAgenda({ workspace }: { workspace?: WorkspaceDocument }) {
  ...
  return <section className="header-agenda" aria-label={ru ? 'Сейчас и далее' : 'Now and next'}>
    ...
    {next && <span className="header-agenda-part"><span>{next.kind === 'due' ? (ru ? 'до due' : 'due in') : (ru ? 'через' : 'in')} {formatAgendaRemaining(next.at - now, ru ? 'ru' : 'en')} ·</span><span className="header-agenda-title" title={agendaPlainText(next.title, ru)}><AgendaTitle text={next.title} ru={ru} /></span></span>}
