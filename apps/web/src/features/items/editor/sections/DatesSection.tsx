...
export function DatesSection({ item, workspace, sectionMark, patchSchedule, scheduledDuration, patchScheduledDuration, applyDurationPreset, timezoneOpen, setTimezoneOpen }: Props) {
  ...
  return <ItemSection sectionKey="dates" title="Dates & time" filledMark={sectionMark(Boolean(item.schedule?.availableFrom || item.schedule?.startAt || item.schedule?.endAt || item.schedule?.dueAt || item.schedule?.estimatedDuration || item.schedule?.allDay))}>
    ...
    <Disclosure uiKey="item-editor:available-from" summary={<>Available to work from <span>Optional</span></>} className="optional-field"><Field label="Available to work from" optional><DateTimeField label="Available to work from" value={item.schedule?.availableFrom} language={language} onChange={(value) => patchSchedule({ availableFrom: value })} help="Earliest intended time to begin; not a deadline." /></Field></Disclosure>
    <div className="form-grid two schedule-grid">
      <Field label="Event opens"><DateTimeField label="Event opens" value={item.schedule?.startAt} language={language} onChange={(value) => patchSchedule({ startAt: value })} help="When it begins and appears in the calendar." /></Field>
      <Field label="Duration" hint={item.schedule?.startAt ? 'Choose a preset or type a value. Changes Event ends; Due stays unchanged.' : 'Set Event opens first.'}><DurationField enabled={Boolean(item.schedule?.startAt)} duration={scheduledDuration} onDurationChange={patchScheduledDuration} onPreset={applyDurationPreset} /></Field>
      <Field label="Event ends"><DateTimeField label="Event ends" value={item.schedule?.endAt} language={language} onChange={(value) => patchSchedule({ endAt: value })} help="When the calendar block ends. Use with Event opens." minValue={item.schedule?.startAt} /></Field>
      <Field label="Due / Active range ends"><DateTimeField label="Due / Active range ends" value={item.schedule?.dueAt} language={language} onChange={(value) => patchSchedule({ dueAt: value })} help="Latest acceptable completion time. Tap the empty field to copy Event opens." onFocus={() => { if (!item.schedule?.dueAt && item.schedule?.startAt) patchSchedule({ dueAt: item.schedule.startAt }); }} minValue={item.schedule?.startAt} /></Field>
    </div>
