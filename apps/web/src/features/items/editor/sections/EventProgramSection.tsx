  ...
  return <Disclosure uiKey={`item-program:${item.id}`} persist={false} summary={`${t('Event program', 'Программа мероприятия')}${blocks.length ? ` · ${blocks.length}` : ''}`} className="event-program">
    ...
    {mode === 'blocks' && blocks.map((block, index) => <div className="program-block" key={block.id}>
      ...
      <div className="program-times">{(['startOffsetSeconds', 'endOffsetSeconds'] as const).map((key, position) => {
        ...
        return <Field key={key} label={position ? t('End', 'Окончание') : t('Start', 'Начало')}><Input type="time" step="1" aria-label={`${position ? 'Block end' : 'Block start'} ${index + 1}`} value={parts.time} onChange={(event) => change(parts.day, event.target.value)} />{parts.day !== 0 || dayFields.has(`${block.id}:${key}`) ? <label className="program-day">{t('Day +', 'День +')}<Input type="number" min={0} step={1} aria-label={`${position ? 'End day' : 'Start day'} ${index + 1}`} value={parts.day} onChange={(event) => { const day = Number(event.target.value); if (Number.isInteger(day) && day >= 0 && day <= 36500) change(day, parts.time); }} /></label> : <Button size="compact" variant="ghost" onClick={() => setDayFields((current) => new Set([...current, `${block.id}:${key}`]))}>{t('+ Day', '+ День')}</Button>}</Field>;
      })}</div>
