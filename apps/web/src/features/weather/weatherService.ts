...
export type WeatherHour = { start: number; end: number; probability: number | null };
...
export function parseForecast(value: unknown): WeatherHour[] {
  const hourly = (value as { hourly?: { time?: unknown[]; precipitation_probability?: unknown[] } })?.hourly;
  ...
  return hourly.time.map((time, i) => {
    ...
    return { start: time * 1000 - HOUR, end: time * 1000, probability: p as number | null };
  });
}
export function usableForecast(state: WeatherState): WeatherHour[] {
  const { settings, forecast, now } = state;
  return settings.enabled && settings.precipitation && settings.location && forecast?.locationKey === locationKey(settings.location) && now >= forecast.fetchedAt && now - forecast.fetchedAt < 6 * HOUR ? forecast.hours : [];
}
...
export function createWeatherService() {
  ...
  const eligible = () => active && state.settings.enabled && state.settings.precipitation && !!state.settings.location;
  const schedule = () => {
  ...
  const refresh = async () => {
    ...
    try {
      const query = new URLSearchParams({ latitude: String(location.latitude), longitude: String(location.longitude), hourly: 'precipitation_probability', forecast_days: '16', timeformat: 'unixtime', timezone: 'GMT' });
  ...
  return {
    ...
    configure(patch: Partial<WeatherSettings>) {
      ...
      const changed = settings.enabled !== state.settings.enabled || settings.precipitation !== state.settings.precipitation || JSON.stringify(settings.location) !== JSON.stringify(state.settings.location);
      if (changed) cancel();
