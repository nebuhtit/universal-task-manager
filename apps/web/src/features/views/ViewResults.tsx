...
export function ViewResults({ view, workspace, evaluation, hiddenItemIds, onEdit, onState, onReorder, calendarTimeOnly = false, celebrationColors = new Map<string, string>() }: {
  ...
  const overdueAgeIndicatorEnabled = renderWorkspace.calendarPreferences.appearance.overdueAgeIndicator;
  ...
  if (renderView.renderer === 'calendar') {
    ...
    return dated.length ? <div className="calendar-strip">{dated.map(({ item, date }) => <article data-utm-due-item-id={canQuickChangeDue(item) ? item.id : undefined} data-utm-due-series-id={item.occurrence?.seriesId} data-utm-due-recurrence-id={item.occurrence?.recurrenceId} className={`calendar-item state-${item.state}${isCelebrating(item) ? ' is-celebrating' : ''}${isExiting(item) ? ' is-exiting' : ''}`} style={celebrationStyle(item)} key={item.id}>{stateControl(item)}<button className="calendar-main" onClick={() => onEdit(item)}><time dateTime={date}>{formatViewDate(date, false, renderWorkspace.calendarPreferences.language)}</time>{fieldContent(item, ['schedule.startAt', 'schedule.dueAt'])}</button></article>)}</div> : <p className="empty">{t('Matching items have no dates.')}</p>;
  }
