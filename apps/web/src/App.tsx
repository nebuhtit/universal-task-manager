    ...
    <section className="drawer" role="dialog" aria-modal="true" aria-label="Item editor">
      ...
      <div className="editor-scroll">
        ...
        <details open={item.relations.some((relation) => relation.type === 'parent')}><summary>Subtasks</summary><div className="details-body">
          ...
          {item.relations.filter((relation) => relation.type === 'parent').map((relation) => <div className="chip" key={relation.id}>{workspace.items[relation.targetId]?.title ?? relation.targetId}<button aria-label="Remove subtask" onClick={() => patchItem({ relations: item.relations.filter((entry) => entry.id !== relation.id) })}><CloseIcon /></button></div>)}
          <div className="inline-row"><select id="subtask-target" defaultValue=""><option value="">Choose a subtask…</option>{Object.values(workspace.items).filter((candidate) => candidate.id !== item.id && !candidate.deletedAt && !item.relations.some((relation) => relation.type === 'parent' && relation.targetId === candidate.id)).map((candidate) => <option value={candidate.id} key={candidate.id}>{candidate.title}</option>)}</select><button className="secondary" onClick={() => { const select = document.getElementById('subtask-target') as HTMLSelectElement; if (select.value) patchItem({ relations: [...item.relations, { id: createId(), targetId: select.value, type: 'parent' }] }); }}>Add subtask</button></div>
        </div></details>
