...
export function ItemCard({ item, onEdit, onState, fields, workspace, now, celebrating = false }: { item: UniversalItem; onEdit: () => void; onState: (state: UniversalItem['state']) => void; fields?: string[]; workspace?: WorkspaceDocument; now?: Date; celebrating?: boolean }) {
  ...
  const organizationValue = (field: string) => {
    const names = field === 'area' || field === 'areas' ? item.areas : field === 'project' || field === 'projects' ? item.projects : null;
    if (!names || !workspace) return null;
    const kind = field === 'area' || field === 'areas' ? 'area' : 'project';
    return <>{names.map((name, index) => <span className="organization-colored-name" style={{ '--organization-accent': organizationAccentFor(workspace, kind, name) } as CSSProperties} key={name}>{index ? ', ' : ''}{name}</span>)}</>;
  };
