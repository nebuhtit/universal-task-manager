...
function FilterBlock({ node, onChange, workspace, projectScope = false, depth = 0 }: { node: Expression; onChange: (node: Expression) => void; workspace: WorkspaceDocument; depth?: number; projectScope?: boolean }) {
  ...
  const localizedField = (field: { path: string; label: string; group: string }) => ru
    ? ({ ...field, label: ({ itemKind: 'Тип item', isTemplate: 'Сохранённый шаблон item (устаревшее имя)', role: 'Техническая роль повторения' } as Record<string, string>)[field.path] ?? field.label, group: ({ 'Item kind': 'Тип item', Advanced: 'Дополнительно' } as Record<string, string>)[field.group] ?? field.group })
    : field;
  ...
  const fields = filterFieldOptions(workspace).map(localizedField).filter((field) => !projectScope || ['project', 'area'].includes(field.path));
  ...
  const updateOperator = (operator: string) => {
    if (!row) return;
    const next = changeVisualOperator(row, operator, workspace.customFields);
    updateRow({ operator: next.operator, value: next.value });
  };
  ...
  return <div className="filter-block" data-depth-tone={depth === 0 ? 0 : ((depth - 1) % 4) + 1}>
    ...
    {isGroup ? <>
      ...
      {row.field === schedulePeriodField ? <SchedulePeriodEditor value={row.value} onChange={(value) => updateRow({ value })} /> : row.field === reminderPeriodField ? <ReminderPeriodEditor value={row.value} onChange={(value) => updateRow({ value })} /> : <>
        ...
        {!['is set', 'is not set'].includes(row.operator) && <Field label={t('Value', 'Значение')}>{isOrganizationChoiceField(row.field) ? <SearchableDisclosureList uiKey={`filter-program:${uiId}`} summary={row.value || t('Choose values…', 'Выберите значения…')} items={orgOptions(row.field)} getSearchText={(value) => value} searchLabel={t('Search values', 'Поиск значений')} searchPlaceholder={t('Search', 'Поиск')} renderItem={(value) => <Checkbox label={value} checked={row.value.split(',').map((entry) => entry.trim()).includes(value)} onChange={(event) => { const selected = row.value.split(',').map((entry) => entry.trim()).filter(Boolean); updateRow({ value: (event.target.checked ? [...selected, value] : selected.filter((entry) => entry !== value)).join(', ') }); }} />} /> : visualOptionsForField(row.field, workspace.customFields) ? <Select value={row.value} onChange={(event) => updateRow({ value: event.target.value })}>{!visualOptionsForField(row.field, workspace.customFields)!.includes(row.value) && <option value={row.value}>{row.value || t('Choose value…', 'Выберите значение…')}</option>}{visualOptionsForField(row.field, workspace.customFields)!.map((value) => <option value={value} key={value}>{visualFilterValueLabel(row.field, value, ru)}</option>)}</Select> : <Input value={row.value} type={visualFieldKind(row.field, workspace.customFields) === 'number' ? 'number' : visualFieldKind(row.field, workspace.customFields) === 'date' ? 'datetime-local' : 'text'} onChange={(event) => updateRow({ value: event.target.value })} />}</Field>}
      </>}
