...
export function AgendaTitle({ text, ru }: { text: string; ru?: boolean }) {
  if (!text.includes('⇥ ')) return <UserDataText>{text}</UserDataText>;
  const parts = text.split('⇥ ');
  return <span aria-label={text.replaceAll('⇥ ', `${ru ? 'Выезд' : 'Departure'} · `)}>
    {parts.map((part, index) => <span key={index}>{index > 0 && <>
    ...
    </svg>{' '}</>}<UserDataText>{part}</UserDataText></span>)}
  </span>;
}
