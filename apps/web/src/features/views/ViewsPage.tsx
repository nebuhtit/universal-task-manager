  ...
  return <section className="page-section views-page">
    ...
    >
      ...
      <ViewEditorSection sectionKey="visual-setup" title="Visual setup">
        ...
        {editing.renderer !== 'calendar' && editing.resultTypes?.includes('project') && <><h3>Project selection</h3><FilterProgramEditor key={editing.id + ':projects'} projectScope workspace={workspace} source={editing.projectQuery?.source ?? 'true'} onValidityChange={setProjectFilterValid} onChange={(source) => setEditing({ ...editing, projectQuery: { source } })} /></>}
        <h3>Item conditions</h3><FilterProgramEditor key={editing.id} workspace={workspace} source={editing.query.source} python={typeof editing.extensions?.filterPython === 'string' ? editing.extensions.filterPython : undefined} onValidityChange={setFilterValid} onChange={(source, python) => { const rows = parseVisualRows(source, workspace.customFields); setVisualRows(rows ?? []); setVisualDirty(rows === null); setEditing({ ...editing, query: { source }, extensions: { ...editing.extensions, filterPython: python } }); }} /></ViewEditorSection>
