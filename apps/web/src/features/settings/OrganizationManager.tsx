...
export function OrganizationManager({ workspace, commit }: { workspace: WorkspaceDocument; commit: Commit }) {
  return <section className="settings-card organization-manager"><p className="eyebrow">PARA ORGANIZATION</p><h2>Areas and Projects</h2><p>Priority is coarse importance. Drag order resolves ties. Item priority remains its own urgency and is never overwritten.</p><div className="organization-manager-columns"><OrganizationList kind="area" workspace={workspace} commit={commit} /><OrganizationList kind="project" workspace={workspace} commit={commit} /></div></section>;
}
