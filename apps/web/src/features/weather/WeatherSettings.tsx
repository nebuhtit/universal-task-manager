...
export function WeatherSettings({ ru, zone }: { ru: boolean; zone: string }) {
  ...
  useEffect(() => { setLatitude(String(settings.location?.latitude ?? '')); setLongitude(String(settings.location?.longitude ?? '')); }, [settings.location]);
  useEffect(() => {
    if (!settings.enabled) { generation.current++; controller.current?.abort(); setBusy(false); setResults([]); }
    return () => { generation.current++; controller.current?.abort(); };
  }, [settings.enabled]);
  const choose = (location: WeatherLocation) => { generation.current++; controller.current?.abort(); setBusy(false); weatherService.configure({ location }); setResults([]); setError(''); };
  const search = async () => {
    ...
    try {
      ...
      setResults(places); if (!places.length) setError(ru ? 'Город не найден. Можно указать координаты.' : 'No city found. You can enter coordinates.');
  ...
  return <details className="settings-disclosure"><summary>{ru ? 'Погода' : 'Weather'}</summary><section className="settings-card weather-settings">
    ...
    {settings.enabled && <>
      ...
      <p>{ru ? 'Место' : 'Location'}: {settings.location?.name ?? (ru ? 'Выберите город или координаты' : 'Choose a city or coordinates')}. {ru ? 'Часовой пояс календаря' : 'Calendar time zone'}: {zone}</p>
      ...
      <div className="weather-search-results">{results.map((v, i) => <Button key={i} onClick={() => choose(v)}>{v.name} ({v.latitude}, {v.longitude})</Button>)}</div>
      ...
      <Button onClick={saveCoordinates}>{ru ? 'Сохранить координаты' : 'Save coordinates'}</Button>
