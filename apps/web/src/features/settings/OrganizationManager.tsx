...
import {
  ensureAreaDefinition, ensureProjectDefinition, orderedOrganizationNames, reorderOrganization,
...
export function OrganizationManager({ workspace, commit }: { workspace: WorkspaceDocument; commit: Commit }) {
  ...
  return <section className="settings-card organization-manager"><p className="eyebrow">PARA ORGANIZATION</p><h2>Areas, Projects and tags</h2><p>Priority is coarse importance. Drag order resolves Area and Project ties. Item priority remains its own urgency and is never overwritten.</p><div className="organization-manager-columns"><OrganizationList kind="area" workspace={workspace} commit={commit} /><OrganizationList kind="project" workspace={workspace} commit={commit} /></div><div className="organization-fallbacks"><Field label="Items without Area" hint="Used by Area order sorting when Area is empty."><Select value={workspace.organizationPreferences.unassignedAreaPriority} onChange={(event) => commit('Change priority for items without Area', (draft) => { draft.organizationPreferences.unassignedAreaPriority = Number(event.target.value) as 0 | 1 | 2 | 3 | 4; })}>{options}</Select></Field><Field label="Items without Project" hint="Used by Project order sorting when Project is empty."><Select value={workspace.organizationPreferences.unassignedProjectPriority} onChange={(event) => commit('Change priority for items without Project', (draft) => { draft.organizationPreferences.unassignedProjectPriority = Number(event.target.value) as 0 | 1 | 2 | 3 | 4; })}>{options}</Select></Field></div><section className="organization-tags"><h3>Tag priorities</h3><p>When an item has several tags, Tag order uses the highest configured priority.</p><div className="organization-manager-list">{tags.map((tag) => <div className="organization-tag-row" key={tag}><strong>#{tag}</strong><Select aria-label={`Priority for tag ${tag}`} value={workspace.organizationPreferences.tagPriorities[tag] ?? 0} onChange={(event) => commit('Change tag priority', (draft) => { draft.organizationPreferences.tagPriorities[tag] = Number(event.target.value) as 0 | 1 | 2 | 3 | 4; })}>{options}</Select></div>)}{!tags.length && <p className="empty">No tags yet. Add a tag to an item first.</p>}</div></section></section>;
}
