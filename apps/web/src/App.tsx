...
import {
  APP_RELEASED_AT, APP_VERSION, backfillItemCreationVersions, collectScheduledEvents, compileQuery, createId, createItem, evaluateFormulas, makeSeries,
...
function ItemEditor({ initial, workspace, onSave, onDelete, onClose }: {
  ...
  const save = () => {
    ...
    try {
      ...
      if (existing) result.createdWithVersion = existing.createdWithVersion;
      if (recurring) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="drawer" role="dialog" aria-modal="true" aria-label="Item editor">
      ...
      <div className="editor-scroll">
        ...
        <details><summary>System metadata</summary><div className="details-body metadata-grid"><div><span>Created at</span><output><time dateTime={item.createdAt}>{new Date(item.createdAt).toLocaleString()}</time></output></div><div><span>Last modified</span><output><time dateTime={item.updatedAt}>{new Date(item.updatedAt).toLocaleString()}</time></output></div><div><span>Created with</span><output>v{item.createdWithVersion}</output></div><div><span>Schema</span><output>{item.schemaVersion}</output></div><div><span>Item ID</span><output>{item.id}</output></div></div></details>
      </div>
...
function SavedViewSection({ view, workspace, onEditView, onEditItem, onState }: {
  ...
  return <details className="view-section" open={open} onToggle={(event) => setOpen(event.currentTarget.open)}>
    <summary className="view-section-summary"><div><span className="preset blank">{view.renderer}</span><h2>{view.name}</h2><code>{view.query.source}</code><p>{filteredItems(workspace, view).length} matching items</p></div><button className="secondary" onClick={(event) => { event.preventDefault(); event.stopPropagation(); onEditView(); }}>Edit view</button></summary>
