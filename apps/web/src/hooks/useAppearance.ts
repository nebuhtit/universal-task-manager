...
export function useAppearance(appearance: WorkspaceDocument['calendarPreferences']['appearance'] | undefined, active: boolean) {
  useEffect(() => {
    const apply = () => {
  ...
  }, [appearance?.mode, appearance?.lightAt, appearance?.darkAt, active]);
}
