...
export function ViewsPage({ workspace, commit, onEditItem, onState, onOpenCalendar, onAddItem, onExportView, celebrationColors, createRequest = 0, onCreateRequestHandled }: {
  ...
  const save = () => {
    if (!editing || !filterValid || (editing.resultTypes?.includes('project') && !projectFilterValid)) return;
    ...
    try {
      ...
      if (result.projectQuery) validateFilterProgram(parseExpression(result.projectQuery.source.trim() || 'true'));
  ...
  return <section className="page-section views-page">
    ...
    >
      ...
      <ViewEditorSection sectionKey="visual-setup" title="Visual setup">
        <Field label="Result types"><Select aria-label="Result types" value={editing.renderer === 'calendar' ? 'item' : (editing.resultTypes ?? ['item']).join(',')} disabled={editing.renderer === 'calendar'} onChange={(event) => setEditing({ ...editing, resultTypes: event.target.value.split(',') as Array<'item' | 'project'> })}><option value="item">Items</option><option value="project">Projects</option><option value="item,project">Items and Projects</option></Select></Field>
