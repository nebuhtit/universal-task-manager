...
test('low-risk item sections save values and disclosure state', async ({ page }) => {
  ...
  await page.getByPlaceholder('Add new task').fill('Design system item');
  ...
  const taskLink = await openSection(page, 'Task link');
  ...
  await expect(page.getByLabel('Task list')).toHaveValue('Phase 3');
  ...
  for (const title of ['Priority', 'Tags', 'Task link', 'Contexts']) {
...
test('template toggle uses the shared checkbox without changing template semantics', async ({ page }) => {
  await createWorkspace(page);
  await page.getByPlaceholder('Add new task').fill('Reusable template');
  await page.getByPlaceholder('Add new task').press('Enter');
  ...
  await page.getByPlaceholder('Add new task').fill('From template');
  await page.getByPlaceholder('Add new task').press('Enter');
