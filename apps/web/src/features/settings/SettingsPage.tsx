...
function ProductGuide() {
  const russian = typeof navigator !== 'undefined' && navigator.language.toLowerCase().startsWith('ru');
  return <details className="settings-guide-section"><summary>{russian ? 'Идея и сценарии' : 'Idea & scenarios'}</summary><div>{russian ? <><h3>Один workspace для внимания, времени и контекста</h3><p>Universal — локальный рабочий стол для задач, событий, привычек и повторяющихся дел. Один item может одновременно появляться в Today, Calendar, проекте, Area, Tag и собственных Views: Views только отбирают и сортируют данные, не создавая копий.</p><h3>План на день</h3><p>В Today оставьте нужные условия: deadline, активный диапазон или event opens. Добавьте Duration, чтобы видеть занятое и свободное время. После завершения есть короткое окно Undo.</p><h3>Проекты, области и теги</h3><p>Area — постоянная сфера, Project — конкретный результат, Tag — поперечный контекст. Unified priority выбирает самую высокую из всех совпавших связей item.</p><h3>Ресурс и повторения</h3><p>Создавайте recurring items для сна, работы и регулярных занятий; добавляйте их в reserved items View, чтобы статистика вычитала их Duration из периода. Repeat after completion считает следующий цикл от фактического выполнения.</p><h3>Календарь и напоминания</h3><p>Calendar — дневной список, а не отдельная база. Его фильтр определяет, какие пересечения Schedule попадают в выбранный день. Reminders напоминают о моменте, но не меняют план и не создают новый item.</p><h3>Безопасность</h3><p>Workspace хранится локально и зашифрован. Регулярно сохраняйте <code>.utmb</code> в Files или iCloud и не передавайте пароль, backup или расшифрованный JSON третьим лицам.</p></> : <><h3>One workspace for attention, time and context</h3><p>Universal is a local-first workspace for tasks, events, habits and repeating work. One item can appear in Today, Calendar, a Project, an Area, a Tag and custom Views: Views only filter and sort data; they never create copies.</p><h3>Daily focus</h3><p>Keep the relevant conditions in Today: deadline, active range or event opens. Add Duration to see reserved and free time. Completing an item includes a short Undo window.</p><h3>Projects, Areas and Tags</h3><p>An Area is an ongoing responsibility, a Project is a concrete outcome, and a Tag is a cross-cutting context. Unified priority selects the highest matching relationship across all of them.</p><h3>Capacity and recurrence</h3><p>Create recurring items for sleep, work and regular commitments; add them as reserved items in a View so statistics subtract their Duration from the period. Repeat after completion calculates the next cycle from the real completion time.</p><h3>Calendar and reminders</h3><p>Calendar is a daily list, not a separate database. Its filter defines which Schedule intersections appear on the selected day. Reminders help you remember a moment; they do not alter the plan or create a new item.</p><h3>Safety</h3><p>The workspace is local and encrypted. Save <code>.utmb</code> backups regularly to Files or iCloud, and do not share the password, backup, or readable JSON with third parties.</p></>}</div></details>;
}
