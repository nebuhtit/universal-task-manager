...
export default function App() {
  ...
  const saveQuickDue = async (selected: { itemId: string; seriesId?: string; recurrenceId?: string }, at: string) => {
    ...
    try {
      ...
      const source = workspace && resolveQuickDueItem(workspace, selected);
      const plannedDateOnly = Boolean(dayOnly && source?.schedule?.plannedDate && !source.schedule.startAt && !source.schedule.dueAt);
      const dueValue = dayOnly && !plannedDateOnly ? dueDateOnlyToIso(at, source ? itemTimeZone(source) : workspace?.calendarPreferences.timezone ?? Intl.DateTimeFormat().resolvedOptions().timeZone) : at;
      if (!dueValue || (dayOnly && new Date(`${at}T12:00:00Z`).toISOString().slice(0, 10) !== at)) throw new Error('Choose a valid calendar date.');
      if (!plannedDateOnly && Date.parse(dueValue) <= actionNow.getTime()) throw new Error('Choose a future Due time.');
      const saved = commit('Move item Due', (draft) => {
        ...
        if (target.schedule?.startAt && Date.parse(dueValue) < Date.parse(target.schedule.startAt)) throw new Error('Due cannot be before Event opens.');
        const updated = syncQuickEntrySource(target, { ...clean(target), schedule: { timezone: itemTimeZone(target), ...target.schedule, ...(plannedDateOnly ? { plannedDate: at } : { dueAt: dueValue, dueDateOnly: dayOnly }) }, updatedAt: actionNow.toISOString(), revision: target.revision + 1 });
