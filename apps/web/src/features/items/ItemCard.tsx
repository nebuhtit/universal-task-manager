...
export function ItemCard({ item, onEdit, onState, fields, workspace, now, viewScripts = [], calendarTimeOnly = false, calendarReference = false, celebrating = false }: { item: UniversalItem; onEdit: () => void; onState: (state: UniversalItem['state']) => void; fields?: string[]; workspace?: WorkspaceDocument; now?: Date; viewScripts?: readonly ItemScriptField[]; calendarTimeOnly?: boolean; calendarReference?: boolean | undefined; celebrating?: boolean }) {
  ...
  return <article data-utm-item-id={item.id} data-utm-series-id={item.occurrence?.seriesId} data-utm-recurrence-id={item.occurrence?.recurrenceId} data-utm-due-item-id={canQuickChangeDue(item) ? item.id : undefined} data-utm-due-series-id={item.occurrence?.seriesId} data-utm-due-recurrence-id={item.occurrence?.recurrenceId} className={`item-card state-${item.state}${celebrating ? ' is-celebrating' : ''}${optimisticClosed === true ? ' is-optimistic-complete' : optimisticClosed === false ? ' is-optimistic-reopen' : ''}`}>
