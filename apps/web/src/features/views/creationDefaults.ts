...
export const defaultValueForPath = (workspace: WorkspaceDocument, path: string): unknown => {
  ...
  if (path === 'schedule.estimatedDuration' || path === 'schedule.travelDuration') return 'PT10M'; if (path.startsWith('schedule.') && path.endsWith('At') || path === 'schedule.availableFrom') return new Date().toISOString(); return '';
};
