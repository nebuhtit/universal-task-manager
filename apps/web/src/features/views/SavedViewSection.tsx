...
export function SavedViewSection({ view, workspace, hiddenItemIds, onEditView, onEditItem, onState, onRendererChange: _onRendererChange, onAddItem, onQuickAddItem, onReorderItems, onResetOrder, onOpenChange, initialOpen, celebrationColors, showTechnicalSummary = true, reorderHandle, headerActions, allowAdd = false }: {
  ...
  return <section className={`view-section${open ? '' : ' is-collapsed'}${view.renderer === 'list' || view.renderer === 'table' ? ' is-reorderable' : ''}`} style={viewStyle}>
    <header className="view-section-summary">{reorderHandle && <div className="view-section-reorder">{reorderHandle}</div>}<button type="button" className="view-section-title" aria-label={`${t(`${open ? 'Collapse' : 'Expand'} ${view.name}`)}${metricsSummary ? `. ${metricsSummary.ariaLabel}` : ''}`} aria-expanded={open} onClick={toggleOpen}><h2><UserDataText>{view.name}</UserDataText></h2>{metrics && <ViewMetricsSummary metrics={metrics} language={workspace.calendarPreferences.language} />}</button><div className="view-section-actions">{headerActions}{open && onEditView && <button type="button" className="icon-button view-settings-button" aria-label={t(`Edit ${view.name}`)} title={t('Edit view')} onClick={onEditView}><LineIcon name="settings" /></button>}</div></header>
    {open && <div className="view-section-body">{showTechnicalSummary && <div className="view-query-summary"><code>{view.query.source.trim() || 'All items'}</code>{view.area && <code className="sort-preview">Area: {view.area}</code>}{view.project && <code className="sort-preview">Project: {view.project}</code>}{view.list && <code className="sort-preview">List: {view.list}</code>}{Object.keys(view.creationDefaults ?? {}).length > 0 && <code className="sort-preview">New item defaults: {Object.keys(view.creationDefaults ?? {}).length}</code>}{(view.sortSource || view.sort?.length) && <code className="sort-preview">Sort: {view.sortSource ?? view.sort.map((sort) => `${sort.field} ${sort.direction}`).join(' · ')}</code>}<p>{t(`${matchingItems} matching items`)}</p></div>}{hasManualOrder && <div className="manual-order-bar"><span>{t('Manual order')}</span><button type="button" onClick={onResetOrder}>{t('Reset order')}</button></div>}<div className="view-results-scroll"><ViewResults view={view} workspace={workspace} evaluation={evaluation} hiddenItemIds={hiddenItemIds} onEdit={onEditItem} onState={onState} onReorder={onReorderItems} celebrationColors={celebrationColors} /></div>{canAdd && (onQuickAddItem ? <form className="view-quick-add" data-quick-capture onSubmit={submitQuickAdd}><Input enterKeyHint="done" value={quickTitle} onChange={(event) => setQuickTitle(event.target.value)} placeholder={t(addLabel)} aria-label={t(`Quick ${addLabel.toLowerCase()}`)} /></form> : <button className="view-add-item" type="button" onClick={() => onAddItem(view)}>+ {t(addLabel)}</button>)}</div>}
  </section>;
}
