...
export function AppShell(props: Props) {
  ...
  return <div className={`app-shell page-${page}`}>
    ...
    <main className="content">
      <header className="topbar"><div><HeaderClock {...(workspace ? { workspace } : {})} {...(activeDateLabel ? { fallback: activeDateLabel } : {})} /></div><div className="top-actions">{page === 'home' && <IconButton size="compact" variant="ghost" className="views-add-button" aria-label={t('New view')} title={t('New view')} onClick={props.onNewView}><LineIcon name="plus"/></IconButton>}<IconButton size="compact" variant="ghost" className="notice-button" aria-label={t('Notifications')} aria-expanded={noticeCenterOpen} onClick={props.onToggleNotices} title={t('Notifications')}><LineIcon name="bell"/>{notificationCount > 0 && <b>{notificationCount}</b>}</IconButton><IconButton size="compact" variant="ghost" className="mobile-menu-button" aria-label={t('Open navigation')} aria-expanded={mobileNavOpen} onClick={props.onToggleNavigation}><LineIcon name="menu"/></IconButton></div></header>
      {mobileNavOpen && <>
