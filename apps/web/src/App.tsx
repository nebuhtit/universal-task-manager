...
function ViewsPage({ workspace, commit }: { workspace: WorkspaceDocument; commit: (message: string, mutation: (draft: WorkspaceDocument) => void) => void }) {
  ...
  return <section className="page-section"><div className="page-title"><div><p className="eyebrow">PROGRAMMABLE LISTS</p><h1>Views</h1><p>Visual clauses and text expressions compile to the same safe AST.</p></div><button className="primary" onClick={() => setEditing({ id: createId(), name: 'New view', query: { source: 'state == "open"' }, renderer: 'list', sort: [{ field: 'updatedAt', direction: 'desc' }], fields: ['title', 'state'] })}>+ New view</button></div>
    <div className="card-grid">{Object.values(workspace.views).map((view) => <article className="settings-card" key={view.id}><span className="preset blank">{view.renderer}</span><h3>{view.name}</h3><code>{view.query.source}</code><p>{filteredItems(workspace, view).length} matching items</p><button className="secondary" onClick={() => setEditing(clean(view))}>Edit</button></article>)}</div>
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {page === 'views' && <ViewsPage workspace={workspace} commit={commit} />}
