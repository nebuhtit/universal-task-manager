...
import {
  defaultVisualConditionForField, isReminderVisualField, parseVisualRows, reminderPeriodField, serializeVisualRows, visualFieldKind, visualFilterFieldLabel, visualFilterValueLabel, visualOperators, visualOptionsForField,
    ...
    <ViewEditorSection sectionKey="calendar-day-filter" title="Filter items">
      <fieldset className="query-builder visual-query-builder">
        ...
        {rows.map((row, index) => <div className="visual-condition-row" key={row.id}>
          ...
          <Field label="Value">{row.operator === 'is set' || row.operator === 'is not set' ? <span className="field-hint">No value needed</span> : visualOptionsForField(row.field, workspace.customFields) ? <Select value={row.value} onChange={(event) => updateRow(row.id, { value: event.target.value })}>{visualOptionsForField(row.field, workspace.customFields)!.map((value) => <option key={value} value={value}>{visualFilterValueLabel(row.field, value)}</option>)}</Select> : <Input type={visualFieldKind(row.field, workspace.customFields) === 'date' ? 'datetime-local' : visualFieldKind(row.field, workspace.customFields) === 'number' ? 'number' : 'text'} value={row.value} onChange={(event) => updateRow(row.id, { value: event.target.value })} />}</Field></>}
