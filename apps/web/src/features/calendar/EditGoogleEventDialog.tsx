...
import { canEditGoogleEvent, GOOGLE_EDIT_EXTENSION, GoogleEditConflict, googleEventChanges, googleEventDraft, rebaseGoogleEdit, loadEditableGoogleEvent, moveSingleGoogleEvent, updateSingleGoogleEvent, type GoogleEditOperation } from '../../services/googleCalendarEdit';
...
export function EditGoogleEventDialog({ item, workspace, onClose, onGoogleEditDraft, onGoogleUpdated }: GoogleEditingCallbacks & { item: UniversalItem; workspace: WorkspaceDocument; onClose: () => void }) {
  ...
  const load = () => execute(async () => {
    ...
    const { event, timeZone, calendars: writable } = await loadEditableGoogleEvent(token.accessToken, item.external!.calendarId, item.external!.eventId, accountEmail);
    ...
    const next: GoogleEditOperation = { calendarId: item.external!.calendarId, destinationCalendarId: operation?.destinationCalendarId ?? item.external!.calendarId, eventId: item.external!.eventId, accountEmail, baseline: event, draft: operation ? rebaseGoogleEdit(operation, event, timeZone) : googleEventDraft(event, timeZone) };
  ...
  return <ResponsiveDialog open className="google-create-dialog" title={t('Edit Google event', 'Изменить событие Google')} onOpenChange={(open) => { if (!open && !busy) void close(); }} footer={<><Button disabled={busy} onClick={() => void close()}>{t('Close', 'Закрыть')}</Button>{operation && !conflict && <Button variant="primary" disabled={busy} onClick={() => void save()}>{operation.attempted ? t('Check / retry save', 'Проверить / повторить сохранение') : t('Save in Google', 'Сохранить в Google')}</Button>}</>}>
    ...
    {operation && draft && <fieldset className="google-create-fields" disabled={busy || conflict || Boolean(operation.attempted)}>
      <Field label={t('Calendar', 'Календарь')}><Select value={operation.destinationCalendarId ?? operation.calendarId} onChange={(event) => setOperation({ ...operation, destinationCalendarId: event.target.value, attempted: false })}>{calendars.map((calendar) => <option key={calendar.id} value={calendar.id}>{calendar.summary || calendar.id}</option>)}</Select></Field>
