...
export function AppShell(props: Props) {
  ...
  return <div className={`app-shell page-${page}`}>
    ...
    <main className="content">
      <header className="topbar"><div><span className="top-summary">{activeDateLabel}</span><span className="sync-state"><i /> Encrypted locally</span></div><div className="top-actions">{page === 'home' && <IconButton size="compact" variant="ghost" className="views-add-button" aria-label="New view" title="New view" onClick={props.onNewView}><LineIcon name="plus"/></IconButton>}<IconButton size="compact" variant="ghost" className="notice-button" aria-label="Notifications" aria-expanded={noticeCenterOpen} onClick={props.onToggleNotices} title="Notifications"><LineIcon name="bell"/>{notices.length > 0 && <b>{notices.length}</b>}</IconButton><IconButton size="compact" variant="ghost" className="mobile-menu-button" aria-label="Open navigation" aria-expanded={mobileNavOpen} onClick={props.onToggleNavigation}><LineIcon name="menu"/></IconButton></div></header>
