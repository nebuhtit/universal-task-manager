...
export function timelineData(workspace: WorkspaceDocument, key: string, now: Date) {
  ...
  const desiredPadding = Math.max(86_400_000, ...items.filter(item => item.role === 'series_template').map(effectiveItemDurationMs));
  ...
  const events: TimelineEvent[] = [], allDay: UniversalItem[] = [], undated: UniversalItem[] = [];
  ...
  return {
    day, events: visible, allDay, undated,
