...
export function ViewsPage({ workspace, commit, onEditItem, onState, onOpenCalendar, onAddItem, onExportView, celebrationColors, createRequest = 0, onCreateRequestHandled }: {
  ...
  const [projectFilterValid, setProjectFilterValid] = useState(true);
  const usesProjectFilter = editing?.renderer !== 'calendar' && Boolean(editing?.resultTypes?.includes('project'));
  ...
  const save = () => {
    if (!editing || !filterValid || (usesProjectFilter && !projectFilterValid)) return;
    ...
    try {
      ...
      if (usesProjectFilter && result.projectQuery) validateFilterProgram(parseExpression(result.projectQuery.source.trim() || 'true'));
  ...
  return <section className="page-section views-page">
    ...
    >
      ...
      <ViewEditorSection sectionKey="visual-setup" title="Visual setup">
        <Field label="Result types"><Select aria-label="Result types" value={editing.renderer === 'calendar' ? 'item' : (['item', 'project'] as const).filter((type) => (editing.resultTypes ?? ['item']).includes(type)).join(',')} disabled={editing.renderer === 'calendar'} onChange={(event) => setEditing({ ...editing, resultTypes: event.target.value.split(',') as Array<'item' | 'project'> })}><option value="item">Items</option><option value="project">Projects</option><option value="item,project">Items and Projects</option></Select></Field>
        {editing.renderer !== 'calendar' && editing.resultTypes?.includes('project') && <><h3>Project selection</h3><FilterProgramEditor key={editing.id + ':projects'} projectScope workspace={workspace} source={editing.projectQuery?.source ?? 'true'} onValidityChange={setProjectFilterValid} onApply={workspace.views[editing.id] ? (source) => commit('Apply project filter', (draft) => { if (draft.views[editing.id]) draft.views[editing.id]!.projectQuery = { source }; }) : undefined} onChange={(source) => setEditing({ ...editing, projectQuery: { source } })} /></>}
        <h3>Item conditions</h3><FilterProgramEditor key={editing.id} workspace={workspace} source={editing.query.source} python={typeof editing.extensions?.filterPython === 'string' ? editing.extensions.filterPython : undefined} onValidityChange={setFilterValid} onApply={workspace.views[editing.id] ? (source, python) => commit('Apply item filter', (draft) => { const view = draft.views[editing.id]; if (view) { view.query = { source }; view.extensions = { ...view.extensions, filterPython: python }; } }) : undefined} onChange={(source, python) => { const rows = parseVisualRows(source, workspace.customFields); setVisualRows(rows ?? []); setVisualDirty(rows === null); setEditing({ ...editing, query: { source }, extensions: { ...editing.extensions, filterPython: python } }); }} /></ViewEditorSection>
