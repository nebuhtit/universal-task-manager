  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
    <section className="drawer" role="dialog" aria-modal="true" aria-label="Item editor" onKeyDown={(event) => {
      ...
      <div className="editor-scroll" ref={editorScrollRef}>
        ...
        <ItemSection sectionKey="tags" title="Tags" iconPath="tags" filledMark={sectionMark(commaList(tags).length > 0)} compact><Field label="Tags"><Input aria-label="Tags" value={tags} onChange={(event) => setTags(event.target.value)} placeholder="Add tags separated by commas" /></Field>{collectedTags.length > 0 && <div className="tag-collection" aria-label="Available tags">{collectedTags.map((tag) => <Button size="compact" variant="ghost" className={commaList(tags).includes(tag) ? 'active' : ''} aria-pressed={commaList(tags).includes(tag)} key={tag} onClick={() => toggleTag(tag)}>#{tag}</Button>)}</div>}</ItemSection>
        <ItemSection sectionKey="organization" title="Organization" iconPath="list" filledMark={sectionMark(Boolean(item.area || item.project || item.list))} compact>
          <div className="form-grid three">
            <Field label="Area" optional hint="An ongoing responsibility, such as Work or Health."><Input aria-label="Area" value={item.area ?? ''} list="item-area-values" onChange={(event) => patchItem({ area: event.target.value.trim() || undefined })} placeholder="Optional area" /></Field>
            <Field label="Project" optional hint="A finite outcome. Existing projects can supply their parent Area."><Input aria-label="Project" value={item.project ?? ''} list="item-project-values" onChange={(event) => { const project = event.target.value.trim(); const definition = organizationDefinitionFor(workspace, 'project', project); patchItem({ project: project || undefined, ...(definition && 'area' in definition && definition.area ? { area: definition.area } : {}) }); }} placeholder="Optional project" /></Field>
            <Field label="Task list" optional hint="A plain list is independent from PARA organization."><Input aria-label="Task list" value={item.list ?? ''} list="item-list-values" onChange={(event) => patchItem({ list: event.target.value.trim() || undefined })} placeholder="Optional list" /></Field>
          </div>
        </ItemSection>
