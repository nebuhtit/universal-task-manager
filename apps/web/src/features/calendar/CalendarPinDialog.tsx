...
export function CalendarPinDialog({ workspace, target, onClose, commit, onFlush }: {
  ...
  const choose = (key: string, mode?: 'same_time' | 'queue' | 'parallel') => {
    ...
    if (naturallyOnDay(item, prepared, key)) { setMessage(ru ? 'Item уже присутствует на этом дне. Вторая карточка не добавлена.' : 'This item is already on this day. No second card added.'); return; }
