...
function EntityControls({ kind, name, workspace, commit, onRenamed, onDeleted }: { kind: 'area' | 'project' | 'tag'; name: string; workspace: WorkspaceDocument; commit: Commit; onRenamed: (next: string) => void; onDeleted: () => void }) {
  const [renaming, setRenaming] = useState(false); const [value, setValue] = useState(name);
  const [deleting, setDeleting] = useState(false); const [deleteConfirmation, setDeleteConfirmation] = useState('');
  ...
  const impact = organizationDeletionImpact(workspace, kind, name);
  ...
  if (kind === 'tag' && workspace.calendarPreferences.googleCalendar?.calendars.some((calendar) => calendar.managedTag === name)) return <small>{workspace.calendarPreferences.language === 'ru' ? 'Название и цвет управляются Google Календарём' : 'Name and color are managed by Google Calendar'}</small>;
...
function ProjectAreaLinks({ project, workspace, commit }: { project: string; workspace: WorkspaceDocument; commit: Commit }) {
  ...
  return <section className="organization-project-area-links"><h3>Areas</h3><div className="organization-project-area-chips">{areas.length ? areas.map((area) => <span key={area}><FieldIcon path="area" label="Area" /><UserDataText>{area}</UserDataText><IconButton size="compact" variant="ghost" aria-label={`Remove ${project} from ${area}`} onClick={() => change(areas.filter((candidate) => candidate !== area))}><CloseIcon /></IconButton></span>) : <span className="organization-system-group-note">No Area</span>}</div>{available.length > 0 && <div className="organization-project-link"><Select aria-label={`Add Area to ${project}`} value={nextArea} onChange={(event) => setNextArea(event.target.value)}><option value="">Add Area…</option>{available.map((area) => <option translate="no" data-utm-user-data value={area} key={area}>{area}</option>)}</Select><Button size="compact" disabled={!nextArea} onClick={() => { change([...areas, nextArea]); setNextArea(''); }}>Add</Button></div>}</section>;
}

function TagCatalogRow({ tag, workspace, onOpen }: { tag?: string; workspace: WorkspaceDocument; onOpen: () => void }) {
  return <button type="button" className={`organization-entity-link organization-tag-entry${tag ? '' : ' organization-unassigned'}`} style={tag ? { color: organizationAccentFor(workspace, 'tag', tag) } : undefined} onClick={onOpen}><FieldIcon path="tag" label="Tag" />{tag ? <UserDataText>{tag}</UserDataText> : 'No Tags'}</button>;
}
...
export function OrganizationManager({ workspace, commit, onEditItem = () => {}, onState = () => {}, onAddItem = () => {}, onQuickAddItem = () => {}, onExport, celebrationColors }: { workspace: WorkspaceDocument; commit: Commit; onEditItem?: (item: UniversalItem) => void; onState?: (item: UniversalItem, state: UniversalItem['state'], celebrationColor?: string) => void; onAddItem?: (view: SavedView) => void; onQuickAddItem?: (view: SavedView, title: string) => void; onExport?: () => void; celebrationColors?: ReadonlyMap<string, string> }) {
  ...
  if (route.kind === 'area') {
    ...
    return <section className="organization-detail-page"><header className="organization-detail-header"><Button variant="ghost" onClick={() => setRoute({ kind: 'overview' })}>‹ PARA</Button><h2 style={area ? { color: organizationAccentFor(workspace, 'area', area) } : undefined}><FieldIcon path="area" label="Area" />{area ? <UserDataText>{area}</UserDataText> : 'No Area'}</h2>{area && <EntityControls kind="area" name={area} workspace={workspace} commit={commit} onRenamed={(next) => setRoute({ kind: 'area', area: next })} onDeleted={() => { setOrderDraft(null); setRoute({ kind: 'overview' }); }} />}</header>{!area && <p className="organization-system-group-note">Projects and items that are not linked to an Area.</p>}<div className="organization-detail-views">{views.map((view) => <section className="organization-scoped-view" key={view.id}>{view.project && <div className="organization-scoped-project-controls"><button type="button" className="organization-entity-link organization-project-name" style={{ color: organizationAccentFor(workspace, 'project', view.project) }} onClick={() => setRoute({ kind: 'project', project: view.project! })}><FieldIcon path="project" label="Project" /><UserDataText>{view.project}</UserDataText></button><EntityControls kind="project" name={view.project} workspace={workspace} commit={commit} onRenamed={() => {}} onDeleted={() => setOrderDraft(null)} /></div>}<ScopedView view={view} workspace={workspace} commit={commit} onEditItem={onEditItem} onState={onState} onAddItem={onAddItem} onQuickAddItem={onQuickAddItem} celebrationColors={celebrationColors} /></section>)}</div></section>;
  }
  ...
  return <section className="settings-card organization-manager" aria-label="PARA organization">
    ...
    <section className="organization-priority"><h3>Unified priority</h3><p className="organization-explanation">Every Area, Project and Tag attached to an item competes in this one ladder. Its highest matching row wins; every row remains draggable.</p><DraggablePriorityRows entries={orderedOrganizationPriorityEntries(orderWorkspace)} onReorder={(order) => orderCommit('Reorder unified organization priority', (draft) => reorderOrganizationPriority(draft, order))} render={(entry) => { const accent = organizationAccentFor(orderWorkspace, entry.kind, entry.name ?? undefined); const open = () => entry.kind === 'area' ? setRoute(entry.name === null ? { kind: 'area' } : { kind: 'area', area: entry.name }) : entry.kind === 'project' && entry.name !== null ? setRoute({ kind: 'project', project: entry.name }) : entry.kind === 'tag' ? setRoute(entry.name === null ? { kind: 'tag' } : { kind: 'tag', tag: entry.name }) : undefined; return <><span className={`organization-kind kind-${entry.kind}`}><FieldIcon path={entry.kind} label={entry.kind === 'area' ? 'Area' : entry.kind === 'project' ? 'Project' : 'Tag'} />{entry.kind}</span><span className="organization-priority-label"><button type="button" className={`organization-entity-link ${entry.kind === 'area' ? 'is-area' : entry.kind === 'project' ? 'is-project' : 'is-tag'}${entry.name === null ? ' organization-unassigned' : ''}`} style={accent ? { color: accent } : undefined} onClick={open}>{entry.name === null ? label(entry) : <UserDataText>{entry.kind === 'tag' ? `#${entry.name}` : entry.name}</UserDataText>}</button>{entry.kind === 'project' && entry.name !== null && <small>In {entry.area ? <UserDataText>{entry.area}</UserDataText> : 'No Area'}</small>}</span></>; }} /></section>
    <div className="organization-order-actions"><span>{orderDraft ? 'Unsaved' : 'Saved'}</span><Button size="compact" aria-label="Apply / Save order" disabled={!orderDraft} onClick={saveOrder}>Save</Button><Button size="compact" aria-label="Reset order" variant="secondary" disabled={!orderDraft} onClick={() => setOrderDraft(null)}>Reset</Button></div>
