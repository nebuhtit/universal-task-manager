...
import { boardSettingsFor, completionPhase, moveManualItem, selectViewItems } from './viewSelectors';
import { recordDiagnostic } from '../../services/diagnostics';
import { previewCompletionSound } from '../../hooks/useUiSounds';

const clean = <T,>(value: T): T => JSON.parse(JSON.stringify(value)) as T;
export const VIEW_LIVE_TICK_MS = 1_000;
export const viewNeedsLiveClock = (view: Pick<SavedView, 'fields' | 'scripts'>) => view.fields.some((field) => field === 'scripts' || field.startsWith('script.') || field === 'view_scripts' || field.startsWith('view_script.'));

export function ViewResults({ view, workspace, onEdit, onState, onReorder, celebrationColors = new Map<string, string>() }: {
  view: SavedView; workspace: WorkspaceDocument; onEdit: (item: UniversalItem) => void;
  onState: (item: UniversalItem, state: UniversalItem['state'], celebrationColor?: string) => void; onReorder?: ((itemIds: string[]) => void) | undefined; celebrationColors?: ReadonlyMap<string, string> | undefined;
}) {
