...
export function packageToTabular(value: PortablePackage): TabularWorkbookData {
  ...
  value.items.forEach((item) => {
    ...
    item.reminders.forEach((reminder) => reminders.push({ item_id: item.id, id: reminder.id, mode: reminder.mode, at: reminder.at ?? '', relative_to: reminder.relativeTo ?? '', offset: reminder.offset ?? '', delivery: reminder.delivery ?? 'notification', repeat_every: reminder.repeatEvery ?? '', repeat_until_acknowledged: String(reminder.repeatUntilAcknowledged), acknowledged_at: reminder.acknowledgedAt ?? '', snoozed_until: reminder.snoozedUntil ?? '' }));
