...
export function ViewResults({ view, workspace, evaluation, hiddenItemIds, onEdit, onState, onReorder, reorderIds, movableIds, referenceIds, itemLabels, calendarTimeOnly = false, celebrationColors = new Map<string, string>() }: {
  view: SavedView; workspace: WorkspaceDocument; onEdit: (item: UniversalItem) => void;
  calendarTimeOnly?: boolean;
  itemLabels?: ReadonlyMap<string, string> | undefined;
  reorderIds?: string[] | undefined; movableIds?: ReadonlySet<string> | undefined; referenceIds?: ReadonlySet<string> | undefined;
  ...
  const dragHandle = (item: Pick<UniversalItem, 'id' | 'title'>) => <button
    ...
    disabled={movableIds !== undefined && !movableIds.has(item.id)}
    ...
    onKeyDown={(event: ReactKeyboardEvent<HTMLButtonElement>) => {
      ...
      if (!target) return; event.preventDefault(); event.stopPropagation(); onReorder(moveManualItem(itemIds, item.id, target, direction > 0), item.id);
    }}
    onPointerDown={(event) => {
