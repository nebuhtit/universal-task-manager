...
function CalendarPage({ workspace, commit, onEditItem }: {
  ...
  const events: EventInput[] = projected.map((row) => {
    ...
    return {
      ...
      editable: true, durationEditable: !row.dueOnly, classNames: [`calendar-state-${row.state}`, `calendar-priority-${row.priority ?? 0}`, row.dueOnly ? 'calendar-due-only' : '', selected.has(row.id) ? 'calendar-selected' : ''].filter(Boolean),
      extendedProps: { row },
    };
  });
  ...
  const handleResize = (info: EventResizeDoneInfo) => {
    const row = byId.get(info.event.id); const end = info.event.end; info.revert(); if (!row || !end) return;
    saveUndoPoint();
    commit('Resize calendar item', (draft) => { const item = materializeProjectedOccurrence(draft, row); resizeCalendarItem(draft, item.id, end.toISOString()); });
  };
  ...
  return <section className="calendar-page page-section">
    ...
    {calendarSettingsOpen && <div className="modal-backdrop"><section className="dialog"><header><h2>Calendar settings</h2><button className="icon-button" aria-label="Close calendar settings" onClick={() => setCalendarSettingsOpen(false)}><CloseIcon /></button></header><label>Timezone<input value={preferences.timezone} onChange={(event) => commit('Change calendar timezone', (draft) => { draft.calendarPreferences.timezone = event.target.value; })} /></label><div className="form-grid two"><label>Workday starts<input type="time" value={preferences.workingHours.start} onChange={(event) => commit('Change calendar work hours', (draft) => { draft.calendarPreferences.workingHours.start = event.target.value; })} /></label><label>Workday ends<input type="time" value={preferences.workingHours.end} onChange={(event) => commit('Change calendar work hours', (draft) => { draft.calendarPreferences.workingHours.end = event.target.value; })} /></label><label>Snap minutes<input type="number" min="5" step="5" value={preferences.snapMinutes} onChange={(event) => commit('Change calendar snap', (draft) => { draft.calendarPreferences.snapMinutes = Math.max(5, Number(event.target.value) || 15); })} /></label><label>Default duration<input type="number" min="5" step="5" value={preferences.defaultDurationMinutes} onChange={(event) => commit('Change default duration', (draft) => { draft.calendarPreferences.defaultDurationMinutes = Math.max(5, Number(event.target.value) || 30); })} /></label></div><label className="check"><input type="checkbox" checked={preferences.weekends} onChange={(event) => commit('Toggle calendar weekends', (draft) => { draft.calendarPreferences.weekends = event.target.checked; })} /> Show weekends</label><label>Week starts<select value={preferences.weekStartsOn} onChange={(event) => commit('Change first weekday', (draft) => { draft.calendarPreferences.weekStartsOn = Number(event.target.value) as 0 | 1; })}><option value="1">Monday</option><option value="0">Sunday</option></select></label></section></div>}
  </section>;
}
