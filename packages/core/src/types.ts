...
export const APP_RELEASED_AT = '2026-09-25T12:17:13.238Z';
...
export interface Schedule {
  timezone: string;
  /** Calendar day on which to work, without an implied clock time. */
  plannedDate?: string;
  allDay?: boolean;
  ...
  endAt?: ISODateTime;
  dueAt?: ISODateTime;
  /** A date-only Due is anchored internally at the end of that calendar day. */
  dueDateOnly?: boolean;
  estimatedDuration?: ISODuration;
  /** Time reserved immediately before startAt. It is not part of the task estimate. */
  travelDuration?: ISODuration;
  travelBackDuration?: ISODuration;
...
export interface UniversalItem {
  ...
  isNote?: boolean;
  /** Explicit completion opt-in/out; absent preserves legacy task/event behavior. */
  ...
  eventProgram?: { blocks: EventProgramBlock[] };
...
export interface EventProgramBlock {
  id: string;
  title: string;
...
export interface SavedView {
  /** Missing means legacy item-only results. Projects are derived links, never stored items. */
  resultTypes?: Array<'item' | 'project'>;
  projectQuery?: { source: string };
  id: string;
  name: string;
...
export interface CalendarDayViewPreferences {
  /** Comment-preserving Python-like source; executable filter remains authoritative. */
  filterPython?: string;
  ...
  fields: string[];
  /** Calendar List metadata; falls back to legacy fields for existing workspaces. */
  listFields?: string[];
  /** Timeline card metadata, independent of List. */
...
export interface CalendarSourceReference { itemId: string; seriesId?: string; recurrenceId?: string }
export interface CalendarPin extends CalendarSourceReference { day: string; mode: 'same_time' | 'queue' | 'parallel' }
export interface CalendarPlanningPreferences {
  /** Missing means enabled. Disabling preserves the user's placements. */
  enabled?: boolean;
  orders?: Record<string, string[]>;
  pins?: Record<string, CalendarPin>;
  /** Explicit parallel starts, by day and concrete item/occurrence ID. Sources stay unchanged. */
  parallel?: Record<string, Record<string, string>>;
}

export interface CalendarPreferences {
  planning?: CalendarPlanningPreferences;
  ...
  appearance: { mode: 'system' | 'light' | 'dark' | 'scheduled'; lightAt: string; darkAt: string; tickSound: boolean; uiSound: boolean; overdueAgeIndicator: boolean; headerDueMode?: 'off' | 'timed' | 'all'; /** Records the one-time upgrade that enabled calm sounds by default. */ soundDefaultsVersion?: 1 };
  dayView: CalendarDayViewPreferences;
  timeline?: { mode: 'list' | 'timeline'; hideSleep: boolean; sleepItemId?: string; showUndated?: boolean; showOverdue?: boolean };
  ...
  /** Automatic reminders added only when a new LiveText event is created. */
  liveTextDefaultReminders?: { enabled: boolean; minutesBefore: number[] };
...
export function createWorkspace(name = 'My workspace', now = new Date()): WorkspaceDocument {
  ...
  return {
    ...
    calendarPreferences: {
      ...
      liveTextDefaultReminders: { enabled: true, minutesBefore: [120, 1440] },
    },
