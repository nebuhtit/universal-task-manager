...
test('live text suggestions, correction reports and saved preference', async ({ page }) => {
  ...
  if ((page.viewportSize()?.width ?? 0) <= 950) {
    ...
    await expect(page.locator('.content')).toHaveCSS('padding-top', (page.viewportSize()?.width ?? 0) <= 620 ? '56px' : '64px');
  }
  ...
  const calendarShortcut = page.locator('.capture-dock .live-day-preview');
  ...
  await expect(options.getByRole('option', { name: /^06:/ })).toBeVisible();
  await expect(options.getByRole('option', { name: /^вечером/ })).toHaveCount(0);
  await expect(options.getByRole('option', { name: /^19:/ })).toBeVisible();
  if ((page.viewportSize()?.width ?? 0) <= 620) await options.getByRole('option', { name: /^19:/ }).tap();
  else await options.getByRole('option', { name: /^19:/ }).click();
  await expect(input).toHaveValue('на залив 19.09 19:');
  if ((page.viewportSize()?.width ?? 0) <= 620) await options.getByRole('option', { name: /^:00/ }).tap();
  else await options.getByRole('option', { name: /^:00/ }).click();
  ...
  await page.locator('.capture-dock .live-day-preview').click();
  await expect(page.locator('.calendar-page')).toBeVisible();
  ...
  await expect(page.getByLabel('Event opens date', { exact: true })).toHaveValue(new RegExp(`^${expectedMonth.getFullYear()}-${String(expectedMonth.getMonth() + 1).padStart(2, '0')}-`));
  ...
  await page.getByLabel('Password', { exact: true }).fill('correct horse battery staple'); await page.getByRole('button', { name: 'Unlock', exact: true }).click();
  await navigate(page, 'Home');
