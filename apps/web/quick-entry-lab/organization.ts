...
export function extractOrganization(input: string) {
  ...
  const text = input.replace(pattern, (match, space: string | undefined, key: string | undefined, raw: string | undefined) => {
  ...
  return { text: text.trim(), areas: values.area, projects: values.project, tags: values.tag };
}
