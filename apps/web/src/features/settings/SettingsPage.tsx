...
function ProductGuide({ language }: { language: WorkspaceDocument['calendarPreferences']['language'] }) {
  const commands = <details className="settings-guide-section"><summary>{language === 'ru' ? 'Команды быстрого ввода' : 'Quick entry commands'}</summary>{commandGuide.map(row => <p key={row.en}><strong>{language === 'ru' ? row.ru : row.en}</strong><br /><code>{row.aliases.join(' / ')}</code><br />{language === 'ru' ? row.ruExample : row.enExample}</p>)}</details>;
  const russian = language === 'ru';
  return <details className="settings-guide-section"><summary>{russian ? 'Идея и сценарии' : 'Idea & scenarios'}</summary><div>{commands}{russian ? <><h3>Одно рабочее пространство для внимания, времени и контекста</h3><p>Universal — локальное рабочее пространство для задач, событий, привычек и повторяющихся дел. Один элемент может одновременно появляться в разделе «Сегодня», календаре, проекте, сфере, теге и собственных видах: виды только отбирают и сортируют данные, не создавая копий.</p><h3>План на день</h3><p>В разделе «Сегодня» оставьте нужные условия: срок, активный период или начало события. Укажите длительность, чтобы видеть занятое и свободное время. После завершения действие можно отменить.</p><h3>Проекты, области и теги</h3><p>Сфера — постоянная область ответственности, проект — конкретный результат, тег — общий контекст. Общий приоритет выбирает самую высокую из всех подходящих связей элемента.</p><h3>Ресурс и повторения</h3><p>Создавайте повторяющиеся элементы для сна, работы и регулярных занятий; добавляйте их в резерв времени вида, чтобы статистика учитывала их длительность. Повтор после завершения отсчитывается от фактического выполнения.</p><h3>Календарь и напоминания</h3><p>Календарь — дневной список, а не отдельная база. Его фильтр определяет, какие элементы расписания попадают в выбранный день. Напоминания не меняют план и не создают новых элементов.</p><h3>Безопасность</h3><p>Рабочее пространство хранится локально; в защищённом режиме — зашифрованным. Регулярно сохраняйте <code>.utmb</code> в «Файлах» или iCloud и не передавайте пароль, резервную копию или расшифрованный JSON третьим лицам.</p></> : <><h3>One workspace for attention, time and context</h3><p>Universal is a local-first workspace for tasks, events, habits and repeating work. One item can appear in Today, Calendar, a Project, an Area, a Tag and custom Views: Views only filter and sort data; they never create copies.</p><h3>Daily focus</h3><p>Keep the relevant conditions in Today: deadline, active range or event opens. Add Duration to see reserved and free time. Completing an item includes a short Undo window.</p><h3>Projects, Areas and Tags</h3><p>An Area is an ongoing responsibility, a Project is a concrete outcome, and a Tag is a cross-cutting context. Unified priority selects the highest matching relationship across all of them.</p><h3>Capacity and recurrence</h3><p>Create recurring items for sleep, work and regular commitments; add them as reserved items in a View so statistics subtract their Duration from the period. Repeat after completion calculates the next cycle from the real completion time.</p><h3>Calendar and reminders</h3><p>Calendar is a daily list, not a separate database. Its filter defines which Schedule intersections appear on the selected day. Reminders help you remember a moment; they do not alter the plan or create a new item.</p><h3>Safety</h3><p>The workspace is local and encrypted. Save <code>.utmb</code> backups regularly to Files or iCloud, and do not share the password, backup, or readable JSON with third parties.</p></>}</div></details>;
}
