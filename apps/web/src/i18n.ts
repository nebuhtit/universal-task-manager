...
const en = {
  ...
  'Edit calendar day view': 'Настроить вид дня календаря', 'Close calendar day view editor': 'Закрыть настройки вида дня', 'Calendar and Google Calendar': 'Календарь и Google Календарь', 'Calendar preferences': 'Настройки календаря', 'Selected day': 'Выбранный день',
  ...
  'All scheduled items': 'Все запланированные элементы', 'Search Saved views': 'Поиск по сохранённым видам', 'Search views': 'Поиск видов',
  'Week starts': 'Начало недели', 'Calendar item states': 'Статусы элементов календаря',
  'Google Calendar sync': 'Синхронизация Google Календаря', 'Read-only. Events are mirrored into this workspace; editing opens Google Calendar.': 'Только чтение. События копируются в это пространство; изменение открывает Google Календарь.',
  'This build needs a Google OAuth client ID before connection is available.': 'Для подключения в этой сборке нужен Google OAuth Client ID.', 'Syncing…': 'Синхронизация…', 'Sync now': 'Синхронизировать', 'Connect Google Calendar': 'Подключить Google Календарь', 'Disconnect Google Calendar': 'Отключить Google Календарь', 'Google Calendar all-day event': 'Событие Google Календаря на весь день', 'Connected as': 'Подключено как', 'Last synced': 'Последняя синхронизация', primary: 'основной', 'Show weekends': 'Показывать выходные',
  'Schedule in period': 'Расписание в периоде', 'Schedule period': 'Период расписания', 'Match any': 'Любое условие', 'Match any selected condition (OR)': 'Подходит любое выбранное условие (ИЛИ)', 'Include overdue': 'Включая просроченные', 'Number of days': 'Количество дней', 'Custom period starts': 'Начало периода', 'Custom period ends': 'Конец периода', 'Schedule dates to match': 'Проверяемые даты расписания',
  'Event opens in period': 'Начало события попадает в период', 'Event opens → Event ends overlaps period': 'Событие пересекает период', 'Event opens → Due overlaps period': 'Активный диапазон пересекает период', 'Due in period': 'Срок попадает в период', '+ Add time period': '+ Добавить период',
  'Event opens in day': 'Начало события попадает в день', 'Event opens → Event ends overlaps day': 'Событие пересекает день', 'Event opens → Due overlaps day': 'Активный диапазон пересекает день', 'Due in day': 'Срок попадает в день',
  'Matches any selected schedule condition. Overdue means an unfinished item whose Due is earlier than now.': 'Подходит любое выбранное условие. Просроченным считается незавершённый элемент, у которого срок уже наступил.',
...
const extraTranslations: Record<Exclude<WorkspaceLanguage, 'en'>, Dictionary> = {
  ru: {},
  es: {
    'Match any selected condition (OR)': 'Coincidir con cualquier condición seleccionada (O)',
  ...
  de: {
    'Match any selected condition (OR)': 'Eine beliebige ausgewählte Bedingung erfüllen (ODER)',
  ...
  fr: {
    'Match any selected condition (OR)': 'Correspond à l’une des conditions sélectionnées (OU)',
  ...
  ko: {
    'Match any selected condition (OR)': '선택한 조건 중 하나와 일치 (OR)',
...
Object.assign(extraTranslations.ru, {
  'Item indicators': 'Индикаторы элементов', 'Overdue age': 'Срок просрочки', 'Show overdue hours or days before item properties': 'Показывать часы или дни просрочки перед свойствами элемента', 'Shown for open items that have an overdue Due time without an active range.': 'Показывается для открытых элементов с просроченным сроком Due без активного диапазона.',
...
Object.assign(extraTranslations.ru, {
  ...
  Color: 'Цвет', 'Choose existing Area…': 'Выберите существующую область…', 'Choose existing Project…': 'Выберите существующий проект…',
  'Choose existing list…': 'Выберите существующий список…', 'Or type a new Area': 'Или введите новую область', 'Or type a new Project': 'Или введите новый проект',
  'Or type a new list': 'Или введите новый список', 'Add existing Tag…': 'Добавьте существующий тег…', 'Choose above or type comma-separated Tags': 'Выберите выше или введите теги через запятую',
  ...
  'System group for Projects that are not linked to an Area.': 'Системная группа для проектов, не связанных ни с одной областью.',
...
Object.assign(extraTranslations.ru, {
  ...
  'Time periods': 'Периоды времени', 'Schedule in period': 'Расписание в периоде', 'Next reminder relative to period': 'Ближайшее напоминание относительно периода',
  'Nearest active reminder': 'Ближайшее активное напоминание', 'Has active reminders': 'Есть активные напоминания', 'Next active reminder': 'Ближайшее активное напоминание',
  'Reminder position relative to period': 'Положение напоминания относительно периода', 'Reminder comparison period': 'Период сравнения напоминания',
  'Before period': 'До периода', 'Inside period': 'Внутри периода', 'After period': 'После периода',
  'is set': 'задано', 'is not set': 'не задано', matches: 'соответствует',
  'Uses the nearest active reminder. A reminder without a resolvable date stays active but does not match a period.': 'Используется ближайшее активное напоминание. Напоминание без вычисляемой даты остаётся активным, но не участвует в сравнении с периодом.',
  Statistics: 'Статистика', 'Show time statistics': 'Показывать статистику по времени', 'Always subtract scheduled items': 'Всегда вычитать запланированные элементы', 'Reserved items': 'Постоянные элементы', 'Reserve time': 'Резервировать время', 'Exclude from View': 'Исключить из View', 'Search items': 'Поиск элементов', 'Search reserved items': 'Поиск постоянных элементов', 'No matching items.': 'Совпадений нет.',
  'Capacity period:': 'Период расчёта:', 'Free time unavailable: add one finite Schedule in period rule.': 'Свободное время недоступно: добавьте одно конечное условие «Расписание в периоде».', 'Free time is the whole period minus this view\'s planned Duration and the reserved items below.': 'Свободное время — это весь период за вычетом запланированной длительности элементов вида и выбранных ниже постоянных блоков.', 'Reserve time affects statistics only. Exclude from View adds a visible AND rule to Filter items and Advanced filter code. The two choices are independent.': 'Резервирование времени влияет только на статистику. Исключение из View добавляет видимое условие AND в Filter items и Advanced filter code. Эти настройки независимы.', 'No scheduled items with Duration yet.': 'Пока нет запланированных элементов с длительностью.', '· repeats': '· повторяется',
  'Completion is weighted by Duration. Remaining time includes unfinished items in this view.': 'Процент выполнения взвешивается по длительности. Оставшееся время включает незавершённые элементы этого вида.',
  'Add one finite Schedule in period rule (Today, Tomorrow, week, next days or custom dates) to calculate free time.': 'Добавьте одно конечное условие «Расписание в периоде» (сегодня, завтра, неделя, следующие дни или свои даты), чтобы рассчитать свободное время.',
  'Create a scheduled item with Duration to use it as a fixed capacity block.': 'Создайте запланированный элемент с длительностью, чтобы использовать его как постоянный блок занятости.',
  'A recurring item is counted once for every occurrence inside the view period. If its occurrence is already in the view, it is not subtracted twice.': 'Повторяющийся элемент учитывается для каждого появления внутри периода. Если появление уже есть в виде, второй раз оно не вычитается.',
  ...
  'Duration can be planned without calendar dates.': 'Длительность можно запланировать без календарных дат.',
  'Completed items': 'Выполнено', 'Planned time': 'Плановое время', 'Nearest deadline': 'Ближайший срок', 'No deadline': 'Нет срока',
...
Object.assign(extraTranslations.ru, {
  ...
  'if you remove Universal from the Home Screen, clear website data, or delete the browser profile, the local workspace may be lost. Export an encrypted': 'если удалить Universal с экрана «Домой», очистить данные сайта или удалить профиль браузера, локальное пространство может быть потеряно. Регулярно экспортируйте зашифрованную',
  'backup regularly and keep it in Files, iCloud Drive, or another trusted cloud.': 'резервную копию и храните её в приложении «Файлы», iCloud Drive или другом надёжном облаке.',
...
Object.assign(extraTranslations.es, {
  'Item indicators': 'Indicadores de elementos', 'Overdue age': 'Antigüedad del retraso', 'Show overdue hours or days before item properties': 'Mostrar horas o días de retraso antes de las propiedades', 'Shown for open items that have an overdue Due time without an active range.': 'Se muestra en elementos abiertos con una fecha Due vencida y sin intervalo activo.',
...
Object.assign(extraTranslations.ko, { 'Choose completion time': '완료 시간 선택', 'Completed at': '완료 시각', 'Edit item': '항목 편집' });
...
function translatedRussianDynamic(value: string) {
  const rules: Array<[RegExp, string]> = [
    ...
    [/^(\d+) matching items$/, 'Подходящих элементов: $1'], [/^(\d+) recorded entries$/, 'Записей: $1'], [/^(\d+) selected$/, 'Выбрано: $1'],
