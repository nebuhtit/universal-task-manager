...
import { calculateViewTimeMetrics, effectiveWorkspaceNow, type SavedView, type UniversalItem, type WorkspaceDocument } from '@utm/core';
...
export function SavedViewSection({ view, workspace, onEditView, onEditItem, onState, onRendererChange: _onRendererChange, onAddItem, onQuickAddItem, onReorderItems, onResetOrder, onOpenChange, initialOpen, celebrationColors, showTechnicalSummary = true, reorderHandle, headerActions, allowAdd = false }: {
  ...
  const showTimeStatistics = view.statistics?.showTime !== false;
  const metrics = showTimeStatistics ? calculateViewTimeMetrics(workspace, view, matchingViewItems, effectiveWorkspaceNow(workspace)) : null;
