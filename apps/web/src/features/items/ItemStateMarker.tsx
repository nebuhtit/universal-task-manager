...
export function ItemStateMarker({ item, googleLabel, noteLabel, onOpen, children }: { item: UniversalItem; googleLabel: string; noteLabel: string; onOpen: () => void; children: ReactNode }) {
  ...
  if (isCalendarItem(item)) {
    ...
    return <button type="button" className="external-calendar-state-marker" style={source?.color && /^#[0-9a-f]{6}$/i.test(source.color) ? { color: source.color } : undefined} aria-label={label} title={label} onClick={(event) => { event.stopPropagation(); onOpen(); }}><LineIcon name="calendar" /></button>;
  }
  if (item.isNote) {
