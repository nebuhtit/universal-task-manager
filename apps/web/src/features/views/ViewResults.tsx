...
export function ViewResults({ view, workspace, onEdit, onState, onReorder, celebratingIds = new Set<string>() }: {
  ...
  return <div className="item-list reorderable-item-list">{items.map((item) => <div data-view-item-id={item.id} className={`reorderable-view-item${draggingId === item.id ? ' is-dragging' : ''}${dropTargetId === item.id ? ' is-drop-target' : ''}${isCelebrating(item) ? ' is-celebrating' : ''}`} key={item.id}>{dragHandle(item)}<ItemCard item={item} celebrating={false} fields={visibleFields} workspace={renderWorkspace} now={liveNow} onEdit={() => onEdit(item)} onState={(state) => onState(item, state)} /></div>)}</div>;
}
