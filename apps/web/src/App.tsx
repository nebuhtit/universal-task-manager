...
function ViewsPage({ workspace, commit, onEditItem, onState, onOpenCalendar, onAddItem }: {
  ...
  return <section className="page-section views-page">
    ...
    {editing && <div className="modal-backdrop"><section className="dialog view-editor">
      ...
      <label>Task list<select value={editing.list ?? ''} onChange={(event) => { const list = event.target.value; setEditing(list ? { ...editing, list } : (() => { const { list: _list, ...withoutList } = editing; return withoutList; })()); }}><option value="">All lists</option>{[...new Set(Object.values(workspace.items).map((item) => item.list).filter((list): list is string => Boolean(list)))].sort().map((list) => <option value={list} key={list}>{list}</option>)}</select><small>Only items assigned to this list appear here. A plus button will add directly to it.</small></label>
