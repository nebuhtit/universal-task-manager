...
export function ItemEditor({ initial, workspace, now: suppliedNow, isNew = false, onSave, onDelete, onCreateSubtask, onToggleSubtask, onReadPortableFile, onExportItem, onClose, onHistorySave, onTimerStateSave, onGoogleEditDraft, onGoogleUpdated, onOpenOccurrence }: Partial<GoogleCreationCallbacks & GoogleEditingCallbacks> & {
  ...
  if (googleLink && editingGoogle && onGoogleEditDraft && onGoogleUpdated) return <EditGoogleEventDialog item={googleItem} workspace={workspace} onClose={() => setEditingGoogle(false)} onGoogleEditDraft={onGoogleEditDraft} onGoogleUpdated={async (event, calendarId) => { await onGoogleUpdated(event, calendarId); if (googleEvent) onClose(); else setEditingGoogle(false); }} />;
    ...
    <div className="editor-scroll" ref={editorScrollRef} onFocusCapture={(event) => {
      ...
      save({ dismissKeyboard: true });
        ...
        {quickEntrySource(item) && (sourceEditing ? <section className="description-section" aria-label="Правка быстрого ввода">
          <LiveTextInput multiline placeholder="Строка быстрого ввода" value={sourceDraft} onChange={setSourceDraft} workspace={workspace} workspaceId={workspace.workspaceId} language={workspace.calendarPreferences.language} suggestionsEnabled={workspace.calendarPreferences.liveTextSuggestions !== false} now={now} />
