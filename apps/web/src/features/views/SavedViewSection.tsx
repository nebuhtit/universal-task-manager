...
export function SavedViewSection({ view, workspace, onEditView, onEditItem, onState, onRendererChange: _onRendererChange, onAddItem, onReorderItems, onResetOrder, onOpenChange, initialOpen, celebratingIds, showTechnicalSummary = true }: {
  ...
  return <section className={`view-section${open ? '' : ' is-collapsed'}${view.renderer === 'list' || view.renderer === 'table' ? ' is-reorderable' : ''}`} style={viewStyle}>
    ...
    {open && <div className="view-section-body">{showTechnicalSummary && <div className="view-query-summary"><code>{view.query.source.trim() || 'All items'}</code>{view.list && <code className="sort-preview">List: {view.list}</code>}{Object.keys(view.creationDefaults ?? {}).length > 0 && <code className="sort-preview">New item defaults: {Object.keys(view.creationDefaults ?? {}).length}</code>}{(view.sortSource || view.sort?.length) && <code className="sort-preview">Sort: {view.sortSource ?? view.sort.map((sort) => `${sort.field} ${sort.direction}`).join(' · ')}</code>}<p>{matchingItems} matching items</p></div>}{hasManualOrder && <div className="manual-order-bar"><span>Manual order</span><button type="button" onClick={onResetOrder}>Reset order</button></div>}<div className="view-results-scroll"><ViewResults view={view} workspace={workspace} onEdit={onEditItem} onState={onState} onReorder={onReorderItems} celebratingIds={celebratingIds} /></div>{(view.list || Object.keys(view.creationDefaults ?? {}).length > 0) && <button className="view-add-item" type="button" onClick={() => onAddItem(view)}>{view.list ? `+ Add item to ${view.list}` : '+ Add item'}</button>}</div>}
  </section>;
}
