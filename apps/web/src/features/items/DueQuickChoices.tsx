...
import type { UniversalItem, WorkspaceLanguage } from '@utm/core';
...
export function DueQuickChoices({ item, now, language, onChoose, error }: {
  ...
  const [draft, setDraft] = useState(() => dueWallInput(item.schedule?.dueAt ?? new Date(now.getTime() + 3_600_000).toISOString(), zone));
