    ...
    <section className="lock-card">
      ...
      <details className="install-guide">
        ...
        <div>
          ...
          <p>Your data is stored locally in the browser, not on GitHub. To make a portable copy, open <em>Settings → Encrypted Transfer → Export encrypted .utmb</em>; in the Files/Save dialog choose a folder and confirm. The locked sign-in screen also has <em>Save encrypted recovery copy + log</em>, which creates a <code>.utmlocal</code> copy without unlocking.</p>
          ...
          <p><strong>PC/Mac:</strong> check the browser’s <em>Downloads</em> folder or the folder chosen in the save dialog and search for <code>.utmb</code> or <code>.utmlocal</code>. The encrypted browser database itself is private browser storage and cannot be copied as a normal file; export a backup while the app is available. You can also use <em>Decrypt any UTM backup</em> to create readable JSON without importing it.</p>
          <p><strong>Offline emergency access:</strong> if Universal was opened successfully at least once while online, its installed PWA may have cached the app shell. Open the installed app without a network connection and use <em>Save encrypted recovery copy + log</em> from this Help section. This works only after the service worker finished installing; a first-ever visit while GitHub is already unavailable cannot load the recovery code or extract browser storage.</p>
          <hr />
