...
function ViewsPage({ workspace, commit, onEditItem, onState, onOpenCalendar, onAddItem }: {
  ...
  return <section className="page-section views-page">
    ...
    {editing && <div className="modal-backdrop"><section className="dialog view-editor">
      ...
      <label>Task list<select value={editing.list ?? ''} onChange={(event) => setEditing({ ...editing, list: event.target.value || undefined })}><option value="">All lists</option>{[...new Set(Object.values(workspace.items).map((item) => item.list).filter((list): list is string => Boolean(list)))].sort().map((list) => <option value={list} key={list}>{list}</option>)}</select><small>Only items assigned to this list appear here. A plus button will add directly to it.</small></label>
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="content">
      ...
      {page === 'all' && (() => { const recurringItems = Object.values(workspace.items).filter((item) => item.role === 'series_template' && !item.habit && !item.deletedAt && !isItemTemplate(item)); const templateItems = Object.values(workspace.items).filter((item) => isItemTemplate(item) && !item.deletedAt); return <section className="page-section"><div className="all-sections">{(['open', 'done', 'auto_closed', 'cancelled', 'archived'] as const).map((state) => { const items = Object.values(workspace.items).filter((item) => item.state === state && !item.deletedAt && !isItemTemplate(item) && (item.role !== 'series_template' || Boolean(item.habit)) && !isHabitOccurrence(workspace, item)); return <details key={state} open={readUiBoolean(`all:`, state === 'open' || state === 'auto_closed')} onToggle={(event) => persistUiBoolean(`all:`, event.currentTarget.open)}><summary><span>{stateNames[state]}</span><b>{items.length}</b></summary><div className="item-list">{items.map((item) => <ItemCard key={item.id} item={item} onEdit={() => setEditor(item)} onState={(nextState) => changeItemState(item, nextState)} />)}</div></details>; })}<details open={readUiBoolean('all:templates', templateItems.length > 0)} onToggle={(event) => persistUiBoolean('all:templates', event.currentTarget.open)} className="recurring-items"><summary><span>Templates</span><b>{templateItems.length}</b></summary><div className="item-list">{templateItems.length ? templateItems.map((item) => <ItemCard key={item.id} item={item} onEdit={() => setEditor(item)} onState={(nextState) => changeItemState(item, nextState)} />) : <p className="empty">No templates yet.</p>}</div></details><details open={readUiBoolean('all:recurring', recurringItems.length > 0)} onToggle={(event) => persistUiBoolean('all:recurring', event.currentTarget.open)} className="recurring-items"><summary><span>Recurring items</span><b>{recurringItems.length}</b></summary><p className="section-help">These are the repeating source items. Each scheduled cycle appears separately in the status sections above.</p><div className="item-list">{recurringItems.length ? recurringItems.map((item) => <ItemCard key={item.id} item={item} onEdit={() => setEditor(item)} onState={(nextState) => changeItemState(item, nextState)} />) : <p className="empty">No recurring items yet.</p>}</div></details></div><AllItemsCollections items={Object.values(workspace.items).filter((item) => !item.deletedAt && !isItemTemplate(item) && !isHabitOccurrence(workspace, item))} onEdit={setEditor} onState={changeItemState} /><DeletedItemsList items={deletedItems} onRestore={restoreItem} /></section>; })()}
