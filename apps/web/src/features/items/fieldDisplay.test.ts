...
describe('item field display helpers', () => {
  ...
  it('exposes every normal nested family and item script result to saved views', () => {
    ...
    expect([...paths]).toEqual(expect.arrayContaining([
      ...
      'eventToday', 'eventThisWeek', 'timerHistory', 'external.provider', 'external.calendarId', 'external.connectionId', 'external.eventId',
      'external.transparency', 'external.sourceUrl', 'external.readOnly', 'external.syncedAt',
    ]));
  ...
  it('keeps every user-meaningful item property represented in the View field catalog', () => {
    ...
    // Program state is exposed through its managed Script; a running timer is transient editor state.
    const missing = Object.keys(itemJsonSchema.properties).filter((property) => !['extensions', 'eventProgram', 'activeTimer'].includes(property) && !represented(property));
